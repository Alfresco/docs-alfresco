...
| Conditional | Turn this option on if you would like to link your drop down list with another drop down list and to create a conditional relationship between them. For example, if you select a country from one drop down list, the second drop down list will only show cities from that country. |
| Depends on | Select which other dropdown list you would like to connect with. |
...
1. Add two dropdown list elements to your form.

2. Name one of them Country and the other one City.

3. Select the Country dropdown list element and click the **Advanced** tab.

4. Select **Manual** and turn on the **Conditional** field.

5. Select City from the **Depends on** dropdown list.

6. Select the City dropdown list element and click the **Advanced** tab.

7. Add a name for the label and then add all the Countries using the **Add option** field.

8. Select the Country dropdown list element again and click the **Advanced** tab.
...
The `${my-dropdown-id}` can be used in any position of the URL, for example you can also use `https://mydomain.com/country=${Country}/get-cities`. The result of this configuration is that your app knows the two dropdown lists are linked which means when you select UK from the country list it understands the replacement and calls the correct URL, which in this example would be `https://mydomain.com/get-cities/country=UK`.
