...
[properties reference](TODO:../references/s3-contentstore-ref-config-props.md).

**Basic configuration properties**

1.  Open the `<classpathRoot>/alfresco-global.properties` file.

    If you plan to use IAM roles instead of AWS access and secret keys, ensure you have [configured AWS Identity and Access Management](TODO:s3-contentstore-iam-config.md) correctly before continuing from step [4](TODO:s3-contentstore-config.md#bucketName).

    If you have existing content in a local contentstore (i.e. where Alfresco Content Services is deployed on-premises) and you'd like to transition to using AWS S3 as the only content store, ensure you include the property described in [Configuring S3 Connector on-premises](TODO:../concepts/s3-contentstore-onprem-config.md#on-prem-config) before continuing.
...
7.  If you plan to use the AWS KMS service to manage encryption, you'll need to change the default `s3.encryption` setting.
    ...
    See [Configuring AWS Key Management Service](TODO:s3-contentstore-kms-config.md) for more encryption options.
...
9.  Set the number of days that Amazon S3 should keep the files which are incomplete or aborted uploads, before marking them for deletion:
    ...
    See [Multipart upload overview](TODO:../concepts/s3-contentstore-multipart-upload.md) for more details.
...
10. If you want to apply a tag to your content when it's written into the S3 bucket, you can add the `connector.s3.tagName` and `connector.s3.tagValue` properties.
    ...
    See [Properties reference](TODO:../references/s3-contentstore-ref-config-props.md) for more details.
...
11. Starting from version 3.1, the S3 Connector has the deleted content store disabled by default, since this feature is already present in Amazon's S3 service. For details on how to re-enable it, see [S3 Connector deleted content store](TODO:../concepts/s3-contentstore-delete.md).
...
Follow the steps in [Installing and configuring the S3 Connector](TODO:s3-contentstore-install-intro.md), and the basic 
configuration steps in [Configuring the S3 Connector](TODO:../tasks/s3-contentstore-config.md).
...
1.  Create a new policy.
    ...
    4.  Copy the following content, and replace the bucket name with your bucket name:
        ...
        3.  If lifecycle configuration on the bucket is not required, then see step [8](TODO:s3-contentstore-config.md#step8) in [Configuring the S3 Connector](TODO:s3-contentstore-config.md).
...
For more information about each of these encryption options, see the [Encryption overview](TODO:../concepts/s3-contentstore-encrypt.md).
