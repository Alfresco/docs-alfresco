...
Download the Alfresco Governance Services distribution from [Alfresco Nexus repository](https://nexus.alfresco.com/nexus/#nexus-search;gav~org.alfresco~alfresco-governance*~23.2.0~~){:target="_blank"}.

The Governance Services Community Edition distribution zip contains the following files:

|alfresco-governance-services-community-repo-23.2.0.255.amp|Contains Governance Services functionality that's applied to an existing Alfresco Community Edition installation.|
|alfresco-governance-services-community-share-23.2.0.269.amp|Contains Governance Services functionality that's applied to an existing Alfresco Share installation.|
...
3. Copy the AMP files to the Alfresco `amps` and `amps_share` directories.
    ...
    * Copy the `alfresco-governance-services-community-repo-23.2.0.255.amp` file to the Alfresco `amps` directory.
    * Copy the `alfresco-governance-services-community-share-23.2.0.269.amp` file to the Alfresco `amps_share` directory.
...
5. Run the following commands to install the AMP files:
    ...
    ```bash
    java -jar bin\alfresco-mmt.jar install amps\alfresco-governance-services-community-repo-23.2.0.255.amp tomcat\webapps\alfresco.war
    ```

    ```bash
    java -jar bin\alfresco-mmt.jar install amps_share\alfresco-governance-services-community-share-23.2.0.269.amp tomcat\webapps\share.war
    ```
