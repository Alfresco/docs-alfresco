...
|alfresco-governance-services-community-repo-23.2.1.xxx.amp|Contains Governance Services functionality that's applied to an existing Alfresco Community Edition installation.|
|alfresco-governance-services-community-share-23.2.1.xxx.amp|Contains Governance Services functionality that's applied to an existing Alfresco Share installation.|
...
3. Copy the AMP files to the Alfresco `amps` and `amps_share` directories.
    ...
    * Copy the `alfresco-governance-services-community-repo-23.2.1.xxx.amp` file to the Alfresco `amps` directory.
    * Copy the `alfresco-governance-services-community-share-23.2.1.xxx.amp` file to the Alfresco `amps_share` directory.
...
5. Run the following commands to install the AMP files:
    ...
    ```bash
    java -jar bin\alfresco-mmt.jar install amps\alfresco-governance-services-community-repo-23.2.1.xxx.amp tomcat\webapps\alfresco.war
    ```

    ```bash
    java -jar bin\alfresco-mmt.jar install amps_share\alfresco-governance-services-community-share-23.2.1.xxx.amp tomcat\webapps\share.war
    ```
