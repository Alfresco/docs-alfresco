...
1. Download the Alfresco Content Services 6.1 [docker-compose.yml](https://raw.githubusercontent.com/Alfresco/acs-deployment/support/HF/1.1/docker-compose/docker-compose.yml) file from GitHub.

2. Save the file in a local folder.

3. Edit the file and change the following two services:

    Add a `#` prefix to the Alfresco Content Repository and Alfresco Share Docker image locations so they are commented out, and add the Alfresco Governance image locations, change 3.1.x to latest 3.1 version:

    ```text
    services:
        alfresco:
            #image: alfresco/alfresco-content-repository:6.1.x
            image: quay.io/alfresco/alfresco-governance-repository-enterprise:3.1.x
    ...
    ```text
        share:
            ...
            image: quay.io/alfresco/alfresco-governance-share-enterprise:3.1.x
