...
|alfresco-governance-services-enterprise-repo-7.1.x.amp|Contains Governance Services functionality that's applied to an existing Alfresco Content Services installation.|
|alfresco-governance-services-enterprise-share-7.1.x.amp|Contains Governance Services functionality that's applied to an existing Alfresco Share installation.|
...
1. Browse to the [Support Portal](http://support.alfresco.com){:target="_blank"} and download the Governance Services distribution zip, which contains the following files:
    ...
    * alfresco-governance-services-enterprise-repo-7.1.x.amp
    * alfresco-governance-services-enterprise-share-7.1.x.amp
...
4. Copy the AMP files to the Alfresco `amps` and `amps_share` directories.
    ...
    * Copy the `alfresco-governance-services-enterprise-repo-7.1.x.amp` file to the Alfresco `amps` directory.
    * Copy the `alfresco-governance-services-enterprise-share-7.1.x.amp` file to the Alfresco `amps_share` directory.
...
6. Run the following commands to install the AMP files:
    ...
    ```bash
    java -jar bin\alfresco-mmt.jar install amps\alfresco-governance-services-enterprise-repo-7.1.x.amp tomcat\webapps\alfresco.war
    ```

    ```bash
    java -jar bin\alfresco-mmt.jar install amps_share\alfresco-governance-services-enterprise-share-7.1.x.amp tomcat\webapps\share.war
    ```
