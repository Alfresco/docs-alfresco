...
4. Copy the AMP files to the Alfresco `amps` and `amps_share` directories.
    ...
    * Copy the `alfresco-governance-services-enterprise-repo-3.4.x.amp` file to the Alfresco `amps` directory.
    * Copy the `alfresco-governance-services-enterprise-share-3.4.x.amp` file to the Alfresco `amps_share` directory.
...
6. Run the following commands to install the AMP files:
    ...
    ```bash
    java -jar bin\alfresco-mmt.jar install amps\alfresco-governance-services-enterprise-repo-3.4.x.amp tomcat\webapps\alfresco.war
    ```

    ```bash
    java -jar bin\alfresco-mmt.jar install amps_share\alfresco-governance-services-enterprise-share-3.4.x.amp tomcat\webapps\share.war
    ```
