...
Use the **Logic** filter to match or exclude words, and phrases. Use the **Properties** filter to limit, exclude or exactly match files of a certain size. Use the **Date** filter to find files created or modified within specific times periods. Use the **Location** filter to include locations of where you would like to focus your search results, for example Europe, or Marketing. Use the **Tags** filter to include content that has been tagged, for more see TAGS. Use the **Categories** filter to include content that has been categorized, for more see CATEGORIES. Use the **People** filter to include content created or modified by certain users.
