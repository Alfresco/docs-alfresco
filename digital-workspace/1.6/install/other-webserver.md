...
7. Also in the `app.config.json` file, change the `baseShareUrl` property to be the server name of Alfresco Digital Workspace. For example:
   ...
   > **Note:** If Quickshare is disabled in Alfresco Content Services ignore this step.
    ...
    To disable Quickshare, set `system.quickshare.enabled=false` in the \tomcat\shared\classes\alfresco-global.properties file. For more information, see [Using the alfresco-global.properties file]( link #using-alfresco-globalproperties).
...
> **Note:** When deploying Alfresco Digital Workspace to a different web server, it is recommended you setup Cross-Origin Resource Sharing (CORS). For more information, see [Cross Origin Resource Sharing (CORS) filters]({link #cors-configuration) and [Cross-Origin Resource Sharing (CORS)](https://enable-cors.org/){:target="_blank"}.
