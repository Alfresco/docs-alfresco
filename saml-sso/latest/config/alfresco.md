...
After configuring an identity provider such as [Active Directory]({% link saml-sso/latest/config/adfs.md %}) or [PingFederate]({%link saml-sso/latest/config/ping.md %}), service providers need to be configured for the features of Alfresco you want to enable SAML SSO for: Alfresco Share, the REST API and Alfresco Office Services.
