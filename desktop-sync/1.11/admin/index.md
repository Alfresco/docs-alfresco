...
2. Upload the latest installer files for Windows and Mac clients to this location.
    ...
    See [Updating Desktop Sync]({% link desktop-sync/1.11/upgrade/index.md %}) for more information about how an update is applied.
...
Once users have entered the repository URL (shown in step 2 of [Setting up Desktop Sync]({% link desktop-sync/1.11/install/index.md %}#setting-up-desktop-sync-on-windows) for Windows and [Setting up Desktop Sync]({% link desktop-sync/1.11/install/index.md %}#faq/mac) for Mac), they will be asked to enter their username and password into the SAML provider login page via their default browser.
