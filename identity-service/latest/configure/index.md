...
There are two things that can be configured in the Alfresco Identity Service:
...
Deploying Identity Service will deploy an Alfresco login theme, it can be customized.
...
Customizing a realm manually uses the administrator console of Identity Service to configure realm settings.
...
Customizing a realm using a `JSON` file configures a realm outside of Identity Service and imports it 
...
Deploying Identity Service will deploy an Alfresco login theme.

A custom theme can be applied to the following components of Identity Service:
...
* Use an `emptyDir` that is shared with Identity Service container and configure an `init container` that runs the new theme image and copies it into the theme directory.
