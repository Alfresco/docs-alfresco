...
![LDAP authentication diagram]({% link identity-service/images/ldap.png %})

As shown in the diagram, the Identity Service is used to authenticate the Alfresco Digital Workspace, Alfresco Process Services and Alfresco Process Workspace.
...
* The [correct product versions]({% link identity-service/1.1/support/index.md %}) of the Alfresco software you are using.
...
A client that authenticates against the Identity Service using SAML is required for Alfresco Share.

Clients also need to be created and configured for Alfresco Office Services (AOS) and Desktop Sync if they are used. The configuration steps for these additional clients can be ignored if they are not used.
...
5. Create a new client for Share and a new client for AOS under the `Alfresco` realm or the realm you created in [step 1](#step-1-configure-a-realm-and-client) setting the following for each:
    ...
    *   A valid **Base URL**:
        *   For Share this will be: `https://share.example.com`
        *   For AOS this will be: `https://repo.example.com/alfresco`
    *   An identifiable **Name** that will be displayed.
    *   The **Login Theme** is set to `Alfresco`
    *   Set the **Valid Redirect URIs** using a wildcard `*`:
        *   For Share this will be: `https://share.example.com/*`
        *   or AOS there are two URLS: `https://repo.example.com/alfresco/*` and `https://ids.example.com/*`
    ...
    *   Set **Logout Service POST Binding URL**:
        *   For Share this will be: `https://share.example.com/page/saml-logoutresponse`
        *   For AOS this will be: `https://repo.example.com/alfresco/service/saml/-default-/aos/logout-request`
...
The administration console in Alfresco Content Services (ACS) needs updating for Alfresco Share to use the SAML connection and certificate that was configured in the Identity Service.

Alfresco Office Services (AOS) can also be configured if it is used. The configuration steps for AOS can be ignored if it is not used.
...
| host | APP_CONFIG_OAUTH2_HOST |The address of the Identity Service including the realm name configured in [step 1](#step-1-configure-a-realm-and-client). In the example the realm name is *Alfresco*|
...
## Step 7: (Optional) Configure Alfresco Sync Service
...
## Step 8: Configure Alfresco Process Services
...
## Step 9: Configure Alfresco Process Workspace
...
| identityHost |The address of the Identity Service including the realm name configured in [step 1](#step1-configure-a-realm-and-client). In the example the realm name is **Alfresco**: `https://ids.example.com/auth/realms/alfresco`|
...
| host |The address of the Identity Service including the realm name configured in [step 1](#step1-configure-a-realm-and-client). In the example the realm name is **Alfresco**: `https://ids.example.com/auth/realms/alfresco`|
| clientId |The name of the client configured in [step 1](#step1-configure-a-realm-and-client) for Alfresco Process Workspace, for example `alfresco`|
| implicitFlow ||
