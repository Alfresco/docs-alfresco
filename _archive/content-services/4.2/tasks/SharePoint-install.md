...
3.  Download and extract the alfresco-enterprise-spp-4.2.8.zip file into a relevant directory.

    This file holds the functionality for the SharePoint connector.

4.  Move or copy the SharePoint AMP to the amps directory in your Alfresco installation.

    -   \(Windows\) c:\\Alfresco\\amps
    -   \(Linux\) /opt/alfresco/amps
5.  From the command line, browse to the Alfresco bin directory.

    -   \(Windows\) c:\\Alfresco\\bin
    -   \(Linux\) /opt/alfresco/bin
6.  If you are using Tomcat, you can apply the SharePoint AMP file to the alfresco.war file by running the apply\_amps.\[bat\|sh\] command. Otherwise, use the Module Management Tool.
