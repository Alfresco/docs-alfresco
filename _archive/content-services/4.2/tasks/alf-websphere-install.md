...
-   These instructions are valid for installing on Windows 2008
...
Alfresco does not support CIFS Kerberos authentication on WebSphere. This is because Alfresco relies on Oracle JDK internal classes.

1.  Download the Enterprise EAR file alfresco-enterprise-ear-4.2.8.zip from the Support Portal and extract it to an empty directory.
...
4.  Configure Share to point to the WebSphere default HTTP port 9080 \(or another number that you wish to specify\).
    ...
    1.  Locate the /web-server/classpath/alfresco/web-extension/share-config-custom.xml.sample file from the extracted alfresco-enterprise-ear-4.2.8.zip file.
...
5.  In the web.xml file from the alfresco.war,  remove the `<dispatcher>REQUEST</dispatcher>` parameter from the Global Localization Filter configuration section.
...
7.  Define the environment information using the extension classpath mechanism and the alfresco-global.properties file.
    ...
    1.  Locate the /web-server/classpath/alfresco-global.properties.sample file from the extracted alfresco-enterprise-ear-4.2.8.zip file.
...
9.  Copy the JDBC driver jar to the $\{WAS\_INSTALL\_ROOT\}/lib directory. 
...
10. Install the EAR file.
    ...
    5.  Browse to alfresco-enterprise-4.2.7.ear on the local file system, and then click **Next**.
