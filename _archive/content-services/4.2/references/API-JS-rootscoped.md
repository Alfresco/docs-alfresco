...
|`space`|`org.alfresco.repo.jscript.ScriptNode`|The primary parent ScriptNode for the current node \(ScriptNode\). For a script executing from a rule, the `space` object is the space in which the rule resides. If the rule is inherited, this may not be the expected space.|
...
[Web script reference guide](../concepts/dev-ws-reference.md)

[Surf root objects guide](APISurf-rootscoped.md)
