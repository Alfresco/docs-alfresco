...
-   **ldap.authentication.java.naming.security.authentication**
    ...
    The mechanism to use to authenticate with the LDAP server. Should be one of the standard values documented here or one of the values supported by the LDAP provider. Oracle's LDAP provider supports the SASL mechanisms documented here. Recommended values are:
