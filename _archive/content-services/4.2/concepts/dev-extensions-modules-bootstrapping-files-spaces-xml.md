...
It is possible to bootstrap files and spaces using XML files.
...
In the snippet above, config\_email\_templates.xml contains the actual file references, as well as all file and folder properties. All predetermined files and folders will be placed in the "Company Home/Data Dictionary/Email Templates" space.
...
It is convenient to use a global property value for the property `useExistingStore` in order to control bootstrapping on Alfresco startup. If the store does not yet exist \(the very first time a new alfresco installation starts up\), the data will aways be bootstrapped, no matter the value, but from that moment on the store exists, and setting this to false means the next time Alfresco reboots these files will not be overridden with those on the classpath. Set it to true again and on the next reboot the classpath files are loaded.
