...
-   **fiveStarRatingScheme** - The scheme allows users to rate content on a scale from 1 to 5
 ...
 ![](../images/dev-extensions-repo-rating-share-ui.png)

 In the node actions to the right we will add a document library action called "Rate Node". It will be available if the node has not been previously rated with the Five Star scheme by the user. The action will not be available if the node has been created by current user. This is because Alfresco Rating Service does not allow you to rate what you have created yourself.
 ...
 ![](../images/dev-extensions-repo-rating-share-ui-props.png)

 After a node has been rated the two rating properties will be updated, they show the number of users that have rated the node \(i.e. the count property\) and the total sum of all rating values \(so if 2 users selected 4 stars rating then it will show 8 as in above screenshot\).

 When a user clicks on the 'Rate Node' a pop-up form is displayed as follows:

 ![](../images/dev-extensions-repo-rating-share-ui-select.png)
 ...
 Let's start by defining the [Document Library](../concepts/dev-extensions-share-doclib-actions.md) action \(note that this is not a Repository extension but should be part of a [Share extension AMP](../tasks/alfresco-sdk-tutorials-share-amp-archetype.md)\):
...
```
 ...
 The form for selecting the Five Star rating has been hard-coded with the options as we can see. The Repository action \(with id rate-node\) will receive a parameter called `star_rating` with the selected rating value \(i.e. 1,2,3,4, or 5\).
 ...
 The [Repository Action](dev-extension-points-actions.md) implementation look like this \(this should be contained in a [Repository AMP](../tasks/alfresco-sdk-tutorials-amp-archetype.md)\):
...
```
 ...
 Note that the scheme names are shorter variants of the ones used with the Java Rating Service \(i.e. `fiveStar` instead of `fiveStarRatingScheme`\).

 This sample has shown you how easy it is to start building a rating solution with the out-of-the-box rating schemes. Now, what would you have to do to use a custom rating scheme, such as a TV rating scheme that could be used to rate videos uploaded to Alfresco.
...
```
 ...
 When defining the constructor for this class we map between the REST API name for the scheme and the original scheme name definition, i.e. `usTv` -\> `usTvRatingScheme`.
...
|
|Deployment - App Server|-   A lot of Java code is usually involved in implementing a Rating solution, so it is better to use Repo and Share AMPs for this.

|
|Deployment - SDK Projects: [Repository AMP](../tasks/alfresco-sdk-tutorials-amp-archetype.md) and [Share AMP](../tasks/alfresco-sdk-tutorials-share-amp-archetype.md)|-   **Share:**
-   share-amp/src/main/amp/config/alfresco/web-extension - Doc Lib actions, Properties, Indicators etc
-   share-amp/src/main/amp/web - resources such as CSS, JS, Images
-   share-amp/src/main/java - UI evaluators
-   **Platform:**
-   repo-amp/src/main/java/\{domain specific directory path\} - Rating related Java classes
-   repo-amp/src/main/amp/config/alfresco/module/repo-amp/context/bootstrap-context.xml - Custom Rating Scheme Spring Bean definition, REST API Rating scheme beans, content model bootstrapping
-   repo-amp/src/main/amp/config/alfresco/module/repo-amp/context/service-context.xml - Repo Action Spring Bean definition

|
|More Information| |
|Sample Code|-   [FiveStar Rating sample and custom UsTv Rating sample](https://github.com/Alfresco/alfresco-sdk-samples/tree/alfresco-50/all-in-one/add-rating-repo)
-   [FiveStar Rating UI sample](https://github.com/Alfresco/alfresco-sdk-samples/tree/alfresco-50/all-in-one/add-rating-share)
