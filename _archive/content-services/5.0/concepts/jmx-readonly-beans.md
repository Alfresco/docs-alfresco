...
The following properties are exposed for `Type=ImageMagick` and and `Type=pdf2swf`. `Available` indicates whether the utility is installed correctly and was found when Alfresco started up, and `VersionString` indicates the version information returned by the utility:
