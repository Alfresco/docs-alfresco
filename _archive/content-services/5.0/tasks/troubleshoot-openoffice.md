...
1.  Enable the following log4j properties to debug:
    ...
    For information about how to create a `log4j.properties` file, see [Audit configuration and environment](../concepts/audit-config-env.md).
...
3.  You might see a failure to connect error message, for example:
    ...
    If the OpenOffice process takes more than 10 seconds to fully start up, then Alfresco fails to connect to it. If this occurs, manually kill the soffice.bin process before attempting to restart the Jodconverter subsystem.
...
4.  If the OpenOffice home location is incorrect, the Jodconverter subsystem will still start, but no OpenOffice process will be running or connected. The error is reported in the console but not in the alfresco.log file.
    ...
    -   For other operating systems, it should be set to the directory that contains program/soffice.bin. For example, for Debian/Ubuntu, this is /usr/lib/openoffice, for Fedora, /opt/openoffice.org3, and for Microsoft Windows, C:/Alfresco/OpenOffice.org.
