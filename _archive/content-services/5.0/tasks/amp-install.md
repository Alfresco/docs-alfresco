...
The MMT is included in the Alfresco installers, and it is also available as a separate JAR file from the Alfresco Distribution zip \(alfresco-enterprise-5.0.5.zip\), in the zip's /bin directory. Place the /bin directory and its contents in the same location that is used by the Alfresco installer \(<installdir\>/bin\).
