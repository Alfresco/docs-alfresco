...
4.  Set the Solr search service properties:
    ...
    |--------------------|---------------|-----------|
    |**Content Tracking Enabled**|Yes|This specifies that Solr can still track with the No Index search enabled. This setting can be used to disable Solr tracking by separate Solr instance\(s\) configured to track this server.|
    ...
    |**Solr base URL**|/solr|This specifies the base URL for the Solr web application.|
    ...
    |**Disk Usage \(GB\)**|0|This specifies the disk space used by the latest version of the Solr index. Allow at least double this value for background indexing management.|
