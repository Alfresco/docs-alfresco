...
This section describes how to configure an Oracle database on Amazon RDS for use with Alfresco.
...
5.  Install the Oracle database connector. The database connector allows Oracle database to talk to the Alfresco server.
    ...
    1.  Download `ojdbc7.jar` from the [Oracle download site](http://www.oracle.com/technetwork/database/features/jdbc/index-091264.html).
        ...
        Use the `ojdbc7.jar` from within the Oracle Database 12c Release 1 \(12.1.0.1\) drivers.
...
12. Edit the following line with an absolute path to point to the directory in which you want to store Alfresco data.
