...
1.  Download the following installation file:
    ...
    alfresco-enterprise-5.0.5-installer-linux-x64.bin
...
2.  Execute the downloaded file using the following commands:
    ...
    ```
    chmod 777 alfresco-enterprise-5.0.5-installer-linux-x64.bin 
    ./alfresco-enterprise-5.0.5-installer-linux-x64.bin
    ```

    **Note:** You should avoid running applications as the root \(Linux administrator\) user where possible, however if you must install Alfresco as the root user, then run the alfresco-enterprise-5.0.5-installer-linux-x64.bin file with `sudo` specified:

    ```
    chmod 777 alfresco-enterprise-5.0.5-installer-linux-x64.bin 
    sudo ./alfresco-enterprise-5.0.5-installer-linux-x64.bin
    ```
...
8.  On the Installation Folder window, click Next to accept the default location.
    ...
    For example, the default location is /opt/alfresco-5.0.x.
...
12. On the Alfresco RMI Port window, enter a port number for Alfresco to execute remote commands, and then click **Next**.
...
14. \(Optional\) If you are installing the LibreOffice component, the LibreOffice Server Port window displays. Enter a port number on which the LibreOffice server will listen.

15. On the Warning window, review the list of environment notifications for your installation of Alfresco installation.
...
18. Click **OK** to close the Readme.
    ...
    **Important:** It can take several minutes to start the Alfresco server and to launch Share. Your browser opens and tries to connect to [http://127.0.0.1:8080/share](http://127.0.0.1:8080/share).
...
20. Manually start the Alfresco server:
    ...
    Browse to the installation folder that you created in [8](simpleinstall-enterprise-lin.md#step8). As an administrator, run
