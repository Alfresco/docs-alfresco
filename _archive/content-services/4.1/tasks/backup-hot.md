...
5.  Store both the database and `dir.root` backups together as a single unit.
    ...
    **Note:** If you set `system.content.eagerOrphanCleanup=true` in the alfresco-global.properties file, performing a hot backup is not recommended. Actions taken between the database backup and the file system backup have the potential to cause orphaned nodes in the repository.
