...
-   **ldap.authentication.java.naming.read.timeout**
    ...
    **Note:** This is applicable to versions 4.1.2 or greater.

-   **ldap.synchronization.attributeBatchSize**

    When positive, this property indicates that range retrieval should be used to fetch multi-valued attributes \(such as member\) in batches of the specified size. It also overcomes any size limits imposed by Active Directory.

-   **ldap.synchronization.enableProgressEstimation**

    When true, this property indicates that progress estimation is enabled. When enabled, the user query should be run twice in order to count entries.

-   **ldap.synchronization.groupDisplayNameAttributeName**

    Specifies the attribute on the LDAP group objects that is used to map the authority display name property in Alfresco.
...
-   **ldap.java.naming.ldap.factory.socket**

    The socket factory to ensure that the LDAP SSL certificate is checked. The default is `org.alfresco.repo.security.authentication.ldap.AlfrescoLdapSSLSocketFactory`. Use `AlfrescoLdapSSLSocketFactory` to enable SSL hostname verification. Alternatively, you can specify your own, custom socket factory.
