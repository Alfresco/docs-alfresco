...
3.  Disable SSO by opening the alfresco-global.properties file in a text editor and editing the `ntlm.authentication.sso. enabled` property as follows:
    ...
    -   `ldap.authentication.defaultAdministratorUserNames` — A list of user IDs who should be given administrator privileges by default. Another administrator can include more users as administrators by adding those users to the ALFRESCO\_ADMINISTRATORS group.
