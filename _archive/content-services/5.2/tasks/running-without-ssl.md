...
When you install Alfresco Content Services, port 8443 is automatically configured for SSL communication between Solr and the repository. It is recommended that you configure the Tomcat connector to use SSL and a certificate in the <TOMCAT\_HOME\>/conf/server.xml file as shown below:

```
clientAuth="want"
```

For more information, see [Secure Sockets Layer \(SSL\) and the repository](../concepts/configure-ssl-intro.md).
...
If you decide to turn-off SSL and deactivate authentication between the repository and the Solr server, you need to protect your environment. For more information, see [Configuring SSL for a production environment](configure-ssl-prod.md).
