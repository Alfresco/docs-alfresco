...
-   config/alfresco/module/org\_alfresco\_module\_dod5015/audit/rm-audit.xml: This defines how the data produced by the `AuthenticationService` and the Alfresco Records Management module is persisted. There are some custom `DataGenerator`s and `DataRecorder`s.
