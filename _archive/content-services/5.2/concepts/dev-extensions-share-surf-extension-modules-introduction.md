...
To hide this component, find out the `region-id`, `source-id`, and `scope` so it can be used when defining the extension module. If you are not up to speed on Surf see [Spring Surf deep dive](surf-fwork-intro.md). You can find this information by using a tool called SurfBug \([more info on how to enable this tool](dev-extensions-share-surfbug.md)\). When this tool is enabled, after refreshing the page, red lines will show up and mark the different components on the page:
