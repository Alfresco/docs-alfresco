...
Note that in some cases you must run $ALF\_HOME/scripts/setenv.sh before this command, and to set `$ALF_HOME` and `$CATALINA_HOME` \(as used in $ALF\_HOME/bin/apply\_amps.sh\) is also recommended. `$ALF_HOME` is the installation directory. `$CATALINA_HOME` is the Tomcat installation directory.
