...
```
 ...
 Sometimes we might want the scheduled job to be applied to a set of nodes determined by a query and the job implementation to be in the form of a [Repository Action](dev-extension-points-actions.md). Having the job implemented as a repository action is handy as it can then be re-used in other places. Implementing this kind of scheduled job usually starts off with the repository action class:
...
```
 ...
 The repository action class extends the `ActionExecuterAbstractBase` as usual and implements the `addParameterDefinitions` and `executeImpl` methods that are part of the action interface. See more information about how to implement repository actions [here](dev-extension-points-actions.md). We use the `ServiceRegistry` to get to the public API, such as the `NodeService`. The `actionedUponNodeRef` will contain a node that is part of a result from a query set up in a Spring bean \(we will configure this bean in a bit\).
...
|[Deployment All-in-One SDK project](../concepts/sdk-getting-started.md).|-   Java job implementations: aio/platform-jar/src/main/java/\{custom package path\}
...
|
|More Information|-   [Out-of-the-box Scheduled Jobs](../concepts/scheduled-jobs.md)
