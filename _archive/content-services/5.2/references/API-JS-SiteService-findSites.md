...
This method will find all sites available to the currently authenticated user based on the specified site filter, site preset filter and result set size. The filter parameter will match any sites whose cm:name, cm:title, cm:description *contain* the specified string \(ignoring case\). Note that this method uses [Alfresco Full Text Search](../concepts/rm-searchsyntax-intro.md) to retrieve results and depending on Solr configuration can only offer eventually consistent results.
