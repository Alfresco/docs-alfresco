...
|Description|The repository consists of content, metadata, and index. The content, which is the physical file that you upload to Alfresco, is by default stored in the file system. This is handled by the so called File Content Store, which gets information about where to store files from configuration in the alfresco-global.properties file \(for example, `dir.root`\). It is also possible to store content files in other places, and there are other content store implementations available out-of-the- box to support that:
 ...
 -   **Amazon S3 content store** - store the files in an S3 bucket using the [Alfresco Content Connector for AWS S3](https://docs.alfresco.com/s3connector/concepts/s3-contentstore-overview.html).
...
|
|File Storage Buckets|The default behaviour of the FileContentStore is to store content files in a content URL with the following URL format:
...
|[Deployment All-in-One SDK project](../concepts/sdk-getting-started.md).|-   aio/platform-jar/src/main/java/\{domain specific package\} - all the Java classes making up the custom content store implementation.
