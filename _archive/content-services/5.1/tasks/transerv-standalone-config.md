...
4.  To set up SSL with the Document Transformation Server, update or replace the keystore in the default location: C:\\Program Files \(x86\)\\TransformationServer\\tomcat\\conf\\.keystore, using the method described here: [Configuring SSL for a test environment](configure-ssl-test.md).
