...
This means that you don't need to download a zip file to see its contents. You can quickly upload multiple files at the same time in a zip file, and then unzip them to your preferred Alfresco location.
...
4.  Click **Unzip**.
    ...
    **Note:** If you have a zip that contains several thousand files, it might take more than 10 seconds to complete. If this is the case, you might see a message from Alfresco saying that the unzip operation could not be completed. This message is issued because Alfresco has not had confirmation that the unzip operation has completed within 10 seconds. Check the folder where you wanted to unzip the files, to see if the unzip operation completed successfully.

    **Tip:** Alfresco Administrators can also use the [Bulk Import tool](../concepts/Bulk-Import-Tool.md) to import multiple files into Alfresco.
