...
1.  In the customize-share-header-menu/src/main/amp/web/js folder create a new folder called tutorials.

2.  In the new folder, create a new file SitesMenu.js with the following contents:
...
3.  You need to edit your Surf extension configuration file to ensure that the Dojo AMD loader can locate the new `tutorials` package. Edit customize-share-header-menu/src/main/amp/config/alfresco/web-extension/site-data/extensions/custom-header-extension.xml relpace the contents with the following text:
    ...
    In addition to adding the `tutorials` package to the configuration, note that the file now has the following line:

    ```
    <auto-deploy>true</auto-deploy>
    ```

    This loads your Surf extension automatically when Share starts, so that you no longer need to deploy it manually from the **Module Deployment** page.

4.  Edit the file customize-share-header-menu/src/main/amp/config/alfresco/web-extension/site-webscripts/tutorials/share-header.get.js, created in the previous tutorial, and add the following code at the end of the file:
...
5.  Create a new XML file in the folder customize-share-header-menu/src/main/amp/config/alfresco/web-extension/site-webscripts/tutorials called site-pages.get.desc.xml, with the following content:
...
6.  In the same folder create a new JavaScript file called site-pages.get.js, with the following content:
...
7.  Create a new file in the same folder called site-pages.get.json.ftl, with the following content:
...
8.  Restart the application server by running the [run.sh](../concepts/alfresco-sdk-cmd-reference-aio.md) command to update your Surf extension module.

9.  Now log back in to Alfresco Share and select the **Sites** menu item. Click a site under **Recent Sites**. You will see that there are now additional menu items including **Site Dashboard**, **Document Library**, and **Site Members**. Click on one of these menu items to test it. For example, if you click on the **Site Members** items you will be taken to the **Site Members** page for that site.
