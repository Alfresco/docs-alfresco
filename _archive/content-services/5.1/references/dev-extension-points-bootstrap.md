...
```
 ...
 In this case we are bootstrapping profile information for two users, `alftest1` and `alftest2`. We can store this XML file as part of the Repo AMP project in a file name such as import-user-profiles.xml.
...
```
 ...
 The `password` is stored as an MD4 Hash, which can be generated following the instructions on [this page](http://wiki.alfresco.com/wiki/Security_and_Authentication#How_to_generate_the_correct_MD4_hash).

 Note that while MD4 is the default encoding format for passwords, support for Bcrypt was added in Alfresco 5.1. You can find out more about Bcrypt support [here](../concepts/bcrypt-overview.md).

 We can store this XML file as part of the Repo AMP project in a file name such as import-users.xml.
...
```
 ...
 Besides using patches to bootstrap content there is also something called a `ImporterModuleComponent` that can be used to bootstrap/import content into the Repository, for more information about this see [Module Components](dev-extension-points-module-component.md).
...
-   [Bcrypt password encryption support](../concepts/bcrypt-overview.md)

|
|Sample Code|-   [Sample Repo AMP that bootstraps users, groups, site, file](https://github.com/Alfresco/alfresco-sdk-samples/tree/alfresco-51/all-in-one/bootstrap-content-repo)
