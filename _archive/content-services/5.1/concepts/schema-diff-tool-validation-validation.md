...
The AVM and ACT database objects are all as expected, but there is a difference between the target schema and the ALF \(alf\_ prefixed database objects\) schema reference. Looking at that file it can be seen that an index that is expected to have been auto-generated has been created with an explicit name:
