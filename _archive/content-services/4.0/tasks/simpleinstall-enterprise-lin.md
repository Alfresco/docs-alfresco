...
1.  Download the following installation file:
    ...
    alfresco-enterprise-4.0.2-installer-linux-x64.bin
...
2.  Execute the downloaded file using the following commands:
    ...
    ```
    chmod 777 alfresco-enterprise-4.0.2-installer-linux-x64.bin 
    ./alfresco-enterprise-4.0.2-installer-linux-x64.bin
    ```

    **Note:** You should avoid running applications as the root \(Linux administrator\) user where possible, however if you must install Alfresco as the root user, then run the alfresco-enterprise-4.0.2-installer-linux-x64.bin file with `sudo` specified:

    ```
    chmod 777 alfresco-enterprise-4.0.2-installer-linux-x64.bin 
    sudo ./alfresco-enterprise-4.0.2-installer-linux-x64.bin
    ```
...
8.  On the Installation folder window, click Next to accept the default location.
    ...
    For example, the default location is /opt/alfresco-4.0.x .
