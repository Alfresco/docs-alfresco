...
-   [What is the Icon? file in the Alfresco sync folder?](ds-faqs-mac.md#35)
...
-   [I have changed the site name in Alfresco. Can I continue using Desktop Sync?](ds-faqs-mac.md#24)
...
Sync needs Internet. Syncing will resume when Internet connection is restored.
...
It can be due to internet connection problems, Alfresco connection problems, or synchronization service connection problems; contact your IT team.

By default, changes made to content in Alfresco take five minutes to be synced to your local version, but changes to local files are synced to Alfresco immediately.

If you've made changes locally that are showing in Alfresco, then make sure that you haven't accidentally closed down Desktop Sync by confirming that the ![Menu bar](../images/ico-ds-alfresco.png) icon is showing in the system tray \(Windows\) or menu bar \(Mac\).
...
Yes, you can update the file but your updates will not be synced to Alfresco unless the file is checked in. The file will go to a deferred state.
...
-   If you have an Office file open and you are saving it to your computer.
...
You can continue working on your files offline but they will get synced to Alfresco whenever you have an Internet connection.
...
On a Mac, a file name can't contain the following characters: `\ / : * ? " < >`. Desktop Sync replaces these characters with underscores, for example, Test\>test is replaced by Test\_test on your desktop.
...
**What is the Icon? file in the Alfresco sync folder?**

`Icon?` is a hidden file that's used by Mac OS X to put the Alfresco logo on the Alfresco sync folder in Finder. 
...
Yes. All files synced in Alfresco using Desktop Sync are versioned.
...
Yes, you can continue working with Desktop Sync but the site name won't be updated in the Finder.
...
When a file is renamed in Alfresco to either an unknown extension \(such as `.aaa`\) or to remove the extension, it may not open on your computer. Since the file extension is used to associate programs with files, rename the file to a known extension to fix the file association so you can open the file.
