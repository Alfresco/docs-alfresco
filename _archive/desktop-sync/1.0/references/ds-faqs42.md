...
It can be due to internet connection problems, Alfresco connection problems, or synchronization service connection problems; contact your Alfresco Administrator.
...
It's also possible that someone was updating the content in Alfresco at the same time as you were updating the local content. The file is in conflict state. See Conflict metrics on how to resolve the conflict.
...
Yes, you can update the file but your updates will not synced to Alfresco unless the file is checked-in. The file will go to a deferred state.
...
In Windows, a file name cannot contain the following characters: `\ / : * ? " < >`. Desktop Sync replaces these characters by underscores, for example, Test\>test will be replaced as Test\_test on your desktop.
...
Yes. Any file deleted in Alfresco using Desktop Sync is moved to Alfresco trash can.
