...
-   [As a customer upgrading from a previous version to S3 Connector 2.1, should I remove the useTenantDomainPath property?](s3-contentstore-faq.md#4)
...
The [Alfresco Content Encryption](https://docs.alfresco.com/5.2/concepts/encrypted-overview.html) module can't be applied to the S3 content store. To encrypt content in AWS S3 we recommend using [AWS KMS](https://aws.amazon.com/kms/). To configure the S3 Connector with KMS, follow the steps in [Configuring AWS Key Management Service](../tasks/s3-contentstore-kms-config.md).
