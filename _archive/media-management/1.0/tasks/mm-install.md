...
2.  Unzip the Alfresco Media Management package into a new system directory; for example, opt/media-management:
    ...
    -   If you are using Alfresco 5.0 or later: alfresco-mm-1.0.3.zip
    -   If you are using Alfresco 4.2: alfresco-mm-1.0.3-alf42.zip
    \(where 'n' is the build number for the distribution\)

    You will see four folders:

    -   activemq: contains ActiveMQ software
    -   amps-repository: contains two AMP files to be applied to the Alfresco repository
...
3.  Install the repository AMP file or files. Navigate to the amps-repository directory and copy the file or files to the amps directory.

    There are two AMP files for Alfresco 4.2:

    -   alfresco-messaging-repo-amp-1.0.3-alf42.amp
    -   alfresco-mm-repo-1.0.3-alf42.amp
    There is one AMP file for Alfresco 5.0 or later:

    -   alfresco-mm-repo-1.0.3.amp
4.  Install the Share AMP file. Navigate to the amps\_share directory and copy the provided AMP file to the Alfresco amps\_share directory.

    There is one AMP file. For 4.2:

    -   alfresco-mm-share-1.0.3-alf42.amp
    For 5.0 or later:

    -   alfresco-mm-share-1.0.3.amp
...
7.  Navigate to the bin directory and run the Module Management Tool \(MMT\) file to install the repository AMP files:
    ...
    1.  For the Alfresco repository:
        ...
        where alfresco-mm-<version\>.amp is the specific AMP file that you downloaded. Repeat this if there is more than one AMP file to install for Media Management.
