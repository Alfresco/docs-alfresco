...
where <SyncId\> is a UUID that uniquely represents the Sync Service. This UUID can be obtained from the [Sync Service health check](desktop-sync-monitor.md#healthcheck).
...
|--------|-----------|
|"eventsHealthCheck" : \{    "healthy" : true, "message" : "Ok" \},|This specifies the health of the repository event tracking mechanism.It relies on the `nodeEventLag.99thPercentile` metric. This defines the time taken for events sent by the repository to be consumed by the Sync Service.

It returns `false` if `nodeEventLag.99thPercentile/``1000000000` \> `sync.health.events.eventLagTolerance` \(default value is 5000ms\).
...
\*\*\*Gauge is an instantaneous measurement of a value. See [Gauges](http://metrics.dropwizard.io/3.1.0/getting-started/#gauges).



**Reporting Sync Service metrics to Graphite**

In order to visualize the metrics listed above in a graphical manner, Sync Service can be configured to report all its metrics to [Graphite](http://graphite.readthedocs.io/en/latest/overview.html). See the `sync.metrics.reporter.graphite.*` properties in [Configuring the Sync Service](syncservice-configure.md).

Here you can see a glimpse of how the `timePerCommit` metric looks like in Graphite:

![](../images/ds-monitor-graphite.png)

If one or more Sync Service instances form a cluster, the same metric is reported in the graph as an averaged value.



**Visualizing Sync service metrics with Grafana**
...
![](../images/ds-monitor-metrics-grafana.png)

You can create your own dashboard view with various charts and graphs in Grafana by following the steps in:
