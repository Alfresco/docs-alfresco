...
# Running Sync Service via a script
...
Before you start the Sync Service, see the [Prerequisites](../concepts/desktop-sync-prerequisites.md) and `Readme.txt` file included in the AlfrescoSyncServer-2.2.x.zip for more information.
...
1.  Create a user that you'll use to run the process.
    ...
    2.  For Windows: You only need a dedicated user if JMX remote authentication is enabled. See `Readme.txt` file in AlfrescoSyncServer-2.2.x.zip for details.
