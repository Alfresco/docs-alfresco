...
1.  Hover over a folder/record in the File Plan and click the action that is available to move to the next step in the disposition schedule.
    ...
    |------|-----------------|
    |Cutoff|Cuts off the record/folder and triggers the retention period. Records can't be added to a folder that's been cut off.|
...
|
    |Destroy|Removes the record/folder content from the Records Management system. An audit trail and metadata is retained.|

    **Note:** Each time you manually run an action, that option is replaced with the next action step in the disposition schedule, and a new option for undoing the step you've just done, for example, **Undo Cutoff**.
