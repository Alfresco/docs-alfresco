...
You can add an optional `confidence` field for each key in the property mapping configuration file to override the global configuration. See example in [Custom AI property mapping]({% link intelligence-services/1.1/config/textract.md %}#custom-ai-property-mapping).
...
The above snippet adds the aspects in the earlier [Custom AI content model configuration]({% link intelligence-services/1.1/config/textract.md %}#custom-ai-content-model) (for Textract) to the existing `"ai.metadata.features"` list of items in the `app.extensions.json` file.
