...
5. To secure access to Search and Insight Engine, you must create a new set of keystores and keys.
    ...
    1. Generate secure keys specific to your Alfresco installation. For more information, see [Secure keys]({% link insight-engine/1.0/config/keys.md %}#generate-secure-keys-for-ssl-communication).
...
2. Extract the Search and Insight Engine distribution.
    ...
    By default, the contents of `alfresco-insight-engine-distribution-1.0.0.zip` are decompressed in a root folder as `/alfresco-insight-engine`. See [Search and Insight Engine directory structure]({% link insight-engine/1.0/config/index.md %}#search-and-insight-engine-directory-structure) for more details.
...
3. Configure HTTP.
    ...
    See [Setting up Solr sharding]({% link insight-engine/1.0/config/sharding/create.md %}#core-templates) for a brief description of the out-of-the-box Solr core templates.
...
3. Edit the file and change the Solr 6 service. Add a # prefix to Alfresco Search Services so it is commented out.
    ...
    > **Note:** If you want to use the Apache Zeppelin visualization interface with Search and Insight Engine you have to deploy it using Docker Compose along with Alfresco Content Services, you cannot install it manually. See [Building Reports and Dashboards]({% link insight-engine/1.0/using/index.md %}#Install with Docker Compose) for the additional container information you need to add to your `docker-compose.yml` file.
