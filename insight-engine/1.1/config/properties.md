...
|alfresco.baseUrl|This property configures the base URL to Alfresco Content Services web project, for example `/alfresco`. If you need to change the `baseUrl` value, see [Deploy with a different context path]{% link content-services/6.1/config/repository.md %}#deploy-with-a-different-context-path).|
