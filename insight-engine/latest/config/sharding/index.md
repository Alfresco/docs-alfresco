...
Note that `location` parameter, that was used for previous versions, is not accepted anymore. Solr Backup folder must be specified in `solr.backup.dir` property from `solrcore.properties` file.
