...
Follow the guidance to test the newly installed and configured module for selected SAP Business Objects.

### Material Master

Upload an attachment to a Material Master so it's shown in the **Business Object Types > Products** folder in Content Services.
...
## Supported SAP Business Objects {#SupportedSAPObjects}
...
| ------------ | ------------ | ------------ | ------------- |
| Business Partner | `BUS1006` | `BP` | :heavy_check_mark: |
| Condition Contract | `BUS2235` | `WCOCO` | :heavy_check_mark: |
| Equipment | `EQUI` | `IE03` | :heavy_check_mark: |
| Inbound Delivery | `BUS2015` | `VL33N` | :heavy_check_mark: |
| Material Master | `BUS1001006` | `MM03` | :heavy_check_mark: |
| Outbound Delivery | `LIKP` | `VL03` | :heavy_check_mark: |
| Product | `MARA`, `BUS1001001`, `BUS1001006` | `MM03` | :heavy_check_mark: |
| Production Order  | `PORDER` | `CO03` | :heavy_check_mark: |

## Troubleshooting

Sorry you're having trouble with the Content Connector for SAP Cloud.

Your problem may be related to any one of the following issues.

### RFC destination connection test fails with HTTP response code 400
  
The connection test of the created RFC destination (`SM59`) returns with an **`400`** response code:

![RFC Connection Test fails]({% link sap-cloud/images/sap_no_license_3_rfc_connection.png %}

This may be due to a wrong **Path Prefix** specified for the target system. Make sure you use the exact value mentioned in [step 3 of creating the RFC Connection]({% link sap-cloud/latest/config/index.md %}#new-http-connection).

### RFC destination connection test fails with HTTP response code 402
  
The connection test of the created RFC destination (`SM59`) returns with an **`402`** response code:
...
This happens most probably due to an expired license file. Make sure to have a (valid) license file for the Content Connector for SAP Cloud applied. If the license exists and is valid, this error could also raise due to a missing configuration of the related repository in `OAC0`. In this case, check the repository configuration again.
