...
2. If Kerberos authentication is enabled with SSO, then Share will use cookie-based sessions and you can configure your load balancer to use sticky routing using the `JSESSIONID` cookie.
    ...
    To enable Kerberos with SSO, refer to the instructions in [Configuring authentication]({% link content-services/6.1/admin/auth-sync.md %}#configauthsubsystems) to configure `alfrescoNtlm` or Kerberos authentication, and set (`kerberos.authentication.sso.enabled=true`).

    > **Note:** If you're configuring a cluster, refer to [Setting up clustering]({% link content-services/6.1/admin/cluster.md %}).
...
1. Install the Content Services nodes.
    ...
    See [Installing using distribution zip]({% link content-services/6.1/install/zip/index.md %}) for more information.

2. Set up multiple Content Services instances in a clustered environment.

    See [Setting up clustering]({% link content-services/6.1/admin/cluster.md %}) for more information.
...
2. Enter your user name and password.
    ...
    The Admin Console displays in a browser window. The first page you see is the [System Summary]({% link content-services/6.1/admin/admin-console.md %}#systemsummary).
