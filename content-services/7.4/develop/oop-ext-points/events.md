...
| `repo.event2.mapper.overrideReplacementText` | Allows to change the text displayed in events instead of default: `SENSITIVE_DATA_REMOVED`. **Note**: By default, the values of properties `usr:salt`, `usr:passwordHash`, `usr:password`, and `trx:password` are replaced with the `SENSITIVE_DATA_REMOVED` text.<br><br>Default value: ``
