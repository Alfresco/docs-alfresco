---
title: Setting up authentication and security
---
...
-   **[Alfresco Content Services security](#alfresco-content-services-security)**  
Alfresco Content Services security comprises a combination of authentication and authorization.
-   **[Mitigating brute force attack on user passwords](#mitigating-brute-force-attack-on-user-passwords)**  
Alfresco Content Services 5.2.7 provides basic out-of-the-box protection against brute force attacks on password logins.
-   **[Managing Alfresco keystores](#managing-alfresco-keystores)**  
...
-   **[Cryptographic password hashing](#cryptographic-password-hashing)**  
...
-   **[Authentication subsystems]({% link content-services/5.2/admin/auth-sync.md %})**  
...
-   **[Configuring authentication subsystems]({% link content-services/5.2/admin/auth-sync.md %}#configuring-authentication-subsystems)**  
...
-   **[Configuring synchronization]({% link content-services/5.2/admin/auth-sync.md %}#configuring-synchronization)**  
...
-   **[Managing authentication directories]({% link content-services/5.2/admin/auth-sync.md %}#managing-authentication-directories)**  
...
-   **[Implementation and services](#implementation-and-services)**  
Alfresco Content Services enforces security services for managing authentication information.
-   **[Admin password in default authentication](#admin-password-in-default-authentication)**  
The Admin user password is used by the default authentication system.
-   **[Security policies and filters](#security-policies-and-filters)**  
...
![]({% link content-services/images/warn.png %})
...
Keystores are used also to protect repository/Solr communications using encryption and mutual authentication. In this case, the keystores store RSA keys and certificates. For more information, see [Solr security]({% link content-services/5.2/admin/search.md %}#solr-security).
...
-   **[Keystore key registration](#keystore-key-registration)**  
...
> **Note:** This functionality is not related to [Encrypted node properties](#encrypted-node-properties) (Encrypted node properties will be deprecated and no longer available from Alfresco Content Services 6.0 and above) or [cryptographic password hashing](#cryptographic-password-hashing).
...
-   **[Encrypting configuration properties](#encrypting-configuration-properties)**  
...
1.  Run the Alfresco Encrypted Properties Management Tool.
    ...
    ![]({% link content-services/images/tool.png %})
...
2.  Generate the public and private keys using the `initkey` function. The public and private key pair is stored in the enterprise directory.
    ...
    ![]({% link content-services/images/init.png %})
...
3.  Generate the encrypted string for your password/value using the `encrypt` function.
    ...
    ![]({% link content-services/images/encrypt.png %})
...
4.  Validate that the encrypted value obtained in Step 3 will decrypt the password.
    ...
    ![]({% link content-services/images/validate.png %})
...
-   **[Dynamic authorities and roles](#dynamic-authorities-and-roles)**  
...
-   **[People and users](#people-and-users)**  
...
![]({% link content-services/images/secur-model.png %})

-   **[Application-related zones](#application-related-zones)**  
...
-   **[Authorization-related zones](#authorization-related-zones)**  
...
-   **[Controlling site creation permissions](#controlling-site-creation-permissions)**  
...
-   **[ACL ordering and evaluation](#acl-ordering-and-evaluation)**  
...
![]({% link content-services/images/secur-acl-example.png %})
...
-   **[Public services configuration](#public-services-configuration)**  
...
-   **[Method-level security definition](#method-level-security-definition)**  
...
-   **[Configuring multiple tickets for authentication](#configuring-multiple-tickets-for-authentication)**  
...
-   **[Using guestGroups and adminGroups properties](#using-guestgroups-and-admingroups-properties)**  
...
1.  Run the following query to find out which encoder is being used to store the Admin password. Check that you have only one row in the output.
    ...
    If no rows are returned, set the password using the instructions shown [above](#admin-password-in-default-authentication) (md4 encoding).
...
-   **[Alfresco Share Security policies and filters](#alfresco-share-security-policies-and-filters)**  
You can configure a number of policies and filters in Alfresco Share to mitigate security attacks.
-   **[Alfresco Repository Security policies and filters](#alfresco-repository-security-policies-and-filters)**  
...
-   **[Cross-Site Request Forgery (CSRF) filters for Share](#cross-site-request-forgery-csr-filters-for-share)**  
...
-   **[Iframes and phishing attack mitigation](#iframes-and-phishing-attack-mitigation)**  
...
-   **[Security filters and clickjacking mitigation](#security-filters-and-clickjacking-mitigation)**  
...
-   **[Cross-Site Request Forgery (CSRF) filters for Repository](#cross-site-request-forgery-csrf-filters-for-repository)**  
...
-   **[Cross-Origin Resource Sharing (CORS) filters](#cross-origin-resource-sharing-cors-filters)**  
