...
![A diagram of an Alfresco workflow]({% link content-services/images/wf-process-definition.jpg %})
...
![]({% link content-services/images/wf-arch.jpg %})
...
![A detailed diagram of Alfresc workflow architecture]({% link content-services/images/wf-arch-2.jpg %})
...
![A diagram of Alfresco workflow artifacts]({% link content-services/images/wf-artifacts.jpg %})
...
-   **Resource Bundle (optional)**
    ...
    A workflow resource bundle provides all the human-readable messages displayed in the user interface for managing the workflow. Messages include Task titles, task property names, task choices etc. Alfresco Content Services supports full localization of Alfresco Share, including workflow. Therefore, the same Share resource bundle configuration extends to workflow too.
...
![A diagram of Alfresco workflow tools]({% link content-services/images/wf-tools.jpg %})
...
-   **[The Workflow Console](#the-workflow-console)**  
...
-   **[Enabling workflow process engines](#enabling-workflow-process-engines)**  
...
-   **[The Activiti workflow console](#the-activiti-workflow-console)**  
...
![A diagram of a process definition]({% link content-services/images/wf-process-definition-diag.jpg %})
...
![An image of a process definition xml file]({% link content-services/images/wf-process-definition-diag-2.jpg %})
...
-   **name**
    ...
    is optional and maps to the name property of a ProcessDefinition. The workflow engine itself does not use this property, but it is used in Alfresco Share for displaying the name in a user interface, so you should specify a name.
...
![A diagram of a process definition with two parallel gateways]({% link content-services/images/wf-parallel-diag.jpg %})
...
![A diagram of a process definition with an exclusive gateway]({% link content-services/images/wf-exclusive-diag.jpg %})

### Variables

In a process definition each property in the workflow task model corresponds to a variable available in your workflow.

For example, the Alfresco Content Services supplied BPM task model defines the property **bpm:assignee**. To reference this property in your process definition you would specify the string **bpm_assignee**. Note that the colon character is replaced by an underscore.
...
![A diagram of a process definition showing events where listeners can be configured]({% link content-services/images/wf-process-definition-listeners.jpg %})
...
The following diagram shows an XML fragment from a process definition that contains Alfresco Content Services-specific task listener.

![An XML fragment from a process definition that contains Alfresco-specific task listener]({% link content-services/images/wf-task-listener.jpg %})
...
![An XML fragment from a process definition that contains Alfresco-specific execution listener]({% link content-services/images/wf-execution-listener.jpg %})
...
The client configuration allows for customization of the UI component that is used for presenting workflow‐related information to the user and taking inputs from the user. Alfresco Content Services uses resource bundles to select the text that displays. Resource bundles allow language-specific strings to be used to display information about a workflow or task. The following diagram shows the relationship between the process definition and the task model on the server, and the client configurations and resource bundle in the client.

![An image of the task model in an Alfresco system]({% link content-services/images/wf-task-model.jpg %})
...
Alfresco Content Services ships with two default workflow models that support the default set of process definitions.
...
![The default Alfresco task models]({% link content-services/images/wf-task-model-2.jpg %})
...
![An image of the task model and user interface]({% link content-services/images/wf-task-model-3.jpg %})

-   **[Specifying the task type](#specifying-the-task-type)**  
...
![An image of the formKey xml]({% link content-services/images/wf-task-model-4.jpg %})
...
-   **[Installing Activiti designer](#installing-activiti-designer)**  
