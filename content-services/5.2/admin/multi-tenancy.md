...
-   **[Multi-tenancy administration](#multi-tenancy-administration)**  
...
-   **[Features not supported in a multi-tenant environment](#features-not-supported-in-a-multi-tenant-environment)**  
...
When you install Alfresco Content Services, multi-tenancy is disabled. The multi-tenancy feature is automatically enabled when the first tenant is created.
...
However, if you wish to disable multi-tenancy, you need to delete all the tenants. See [Managing tenants](#managing-tenants) for more information.
...
3.  Perform the following as required:
    ...
    3.  To create a tenant, type `create <tenant domain> <tenant admin password> [<root contentstore dir>]`.
        ...
        ```bash
        create customer_tenant.com l3tm31n /usr/tenantstores/customer_tenant
        ```
...
For more information on administration, see [Using the Admin Console]({% link content-services/5.2/admin/admin-console.md %}) and [Using the Share Admin Tools](#using-the-alfresco-share-admin-tools).
