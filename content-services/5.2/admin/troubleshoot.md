...
-   **[Using the Node Browser](#using-the-node-browser)**  
...
-   **[Troubleshooting IMAP](#troubleshooting-imap)**  
Use this information to troubleshoot IMAP problems.
-   **[Troubleshooting schema-related problems](#troubleshooting-schema-related-problems)**  
...
1.  Click **Admin Tools**, and then click **Node Browser**.
    ...
    The default search type is set to **fts-alfresco**. For most administrative tasks, you can use the default search type. See [Alfresco Full Text Search reference]({% link content-services/5.2/develop/alfresco-full-text-search-ref.md %}#alfresco-full-text-search-reference) for more detail.
...
-   On Linux and Unix environments, you can use `netstat -ln` to check that the correct ports have been opened by the server on the correct network adapters. You can use `telnet <hostname><port>` to check if each open port can be reached by each cluster member.
-   If your cluster members are using NAT and IPv4 addresses, you might need to force the server to listen on IP V4 addresses rather than IP V6. To do this, add:
    ...
    to the startup options of Alfresco Content Services JVM. In a standard Linux/Unix installation, this would require editing of the `JAVA_OPTS` variable in the script, as follow:
    ...
    For more information on the process of initiating clustering and the options available for configuring clustering, see [Setting up clustering]({% link content-services/5.2/admin/cluster.md %}#setting-up-clustering).
...
1.  Enable the following log4j properties to debug:
    ...
    For information about how to create a `log4j.properties` file, see [Key tools and files]({% link content-services/5.2/admin/audit.md %}#key-tools-and-files).
...
3.  You might see a failure to connect error message.
    ...
    If the LibreOffice process takes more than 10 seconds to fully start up, then Alfresco Content Services fails to connect to it. If this occurs, manually kill the `soffice.bin` process before attempting to restart the Jodconverter subsystem.
...
Alfresco Content Services supports NTLM v2 protocol, which is more secure than NTLM v1 protocol. However, NTLM v2 cannot be used with pass-through authentication. You will have to switch to NTLM v1 if you want to use pass-through authentication, where the log on request is passed to an Active Directory or other server to validate the login credentials. For more information, see the [Configuring pass-through]({% link content-services/5.2/admin/auth-sync.md %}#configuring-pass-through-passthru-authentication) topic.
...
-   **[Performing schema dumps](#performing-schema-dumps)**  
Schema dumps are XML representations of the database schema.
-   **[Performing schema validation](#performing-schema-validation)**  
