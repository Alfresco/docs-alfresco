...
-   **[Key tools and files](#key-tools-and-files)**  
...
-   **[How to check audit status](#how-to-check-audit-status)**  
...
-   **[How to use the auditing sample files](#how-to-use-the-auditing-sample-files)**  
...
-   **[Using the auditing ReST API](#using-the-auditing-rest-api)**  
...
-   **[Default auditing global properties](#default-auditing-global-properties)**  
...
-   **[Audit configuration](#audit-configuration-files)**  
...
-   **[Auditing Tutorials](#auditing-tutorials)**  
...
![]({% link content-services/images/auditing-arch.png %})
...
The Audit Service provides a public Java API for interacting with the Audit Component. The Audit Service is described in more detail in the [Public Java API Services documentation]({% link content-services/5.2/develop/reference/java-foundation-ref.md %}#auditservice). The JavaDoc (generated from the Java source code) for the AuditService API can be found [here](http://dev.alfresco.com/resource/docs/java/org/alfresco/service/cmr/audit/AuditService.html).

The AuditService is also exposed via a REST API. This is documented in more detail in the [Using the auditing REST API documentation](#how-to-use-the-auditing-sample-files).

### Audit Filters

Audit Filters filter the raw audit data so that only the required data proceeds further in the pipeline. While some audit filters are [provided by default](#default-auditing-global-properties) you can override these default filters and create new audit filters by editing `alfresco-global.properties`. Audit Filters are described in more detail in the [Audit Filter section](#auditfilters).
...
### Alfresco Content Services log file

The log file, alfresco.log, is found in the directory where Alfresco Content Services is installed. Viewing audit log entries provides a useful way to get a feel for the auditing process.
...
For more information about `curl` and where to find it see this [page]({% link content-services/5.2/develop/rest-api-guide/install.md %}#installing-a-tool-to-make-http-calls).

To check the global status of auditing, such as what audit applications that are enabled, see this [page]({% link content-services/5.2/develop/rest-api-guide/audit-apps.md %}#list-audit-applications).
...
Auditing can also be globally enabled or disabled for Audit applications, see this [page]({% link content-services/5.2/develop/rest-api-guide/audit-apps.md %}#enable-disable-an-audit-application) for more info.
...
In order to use a sample file, remove the .sample extension. It is also assumed you have [enabled auditing]({% link content-services/5.2/admin/audit.md %}#how-to-enable-auditing). You will also need to restart the server so the examples are loaded.

Once the sample files are enabled you can check that the new example audit applications are enabled via the ReST API, see this [page]({% link content-services/5.2/develop/rest-api-guide/audit-apps.md %}#list-audit-applications) for more information.
...
The Audit ReST API covers most of the audit functionality, check it out here in the [user guide]({% link content-services/5.2/develop/rest-api-guide/audit-apps.md %}#managing-audit-applications-and-logs).
...
The default [audit filter](#audit-filters) discards events where the user is `null` or `System`, the content or folder path is under `/sys:archivedItem` or under `/ver:`. The filter also rejects node types other than `cm:folder`, `cm:content` or `st:site`.
...
These properties were introduced in Alfresco One 4.2.4.
...
-   **[Creating new audit filters](#creating-new-audit-filters)**  
...
-   **[Redirected properties](#redirected-properties)**  
It is possible for one property to reference another property.
-   **[Debugging audit filters](#debugging-audit-filters)**  
...
-   **[Deconstructing the sample files](#deconstructing-the-sample-files)**  
...
Or they can be declared in Spring configuration and referenced in the audit configuration (see the [audit-services-context.xml](https://github.com/Alfresco/alfresco-repository/blob/develop/src/main/resources/alfresco/audit-services-context.xml) file), for example:
...
See the documentation on [Using the auditing sample files](#how-to-use-the-auditing-sample-files).
...
#### Login example (alfresco-audit-example-login.xml.sample)
...
#### Data extractors sample (alfresco-audit-example-extractors.xml.sample)
...
The Audit ReST API User Guide can be found [user guide]({% link content-services/5.2/develop/rest-api-guide/audit-apps.md %}#managing-audit-applications-and-logs).

-   **[Understanding PathMappings](#understanding-pathmappings)**  
...
-   **[Using values that have changed in a post method call](#using-values-that-have-changed-in-a-post-method-call)**  
