...
|Description|There is a separate section in the documentation dedicated to [Configuring Alfresco Share]({% link content-services/5.2/develop/share-ext-points/share-config.md %}#configuring-alfresco-share). This configurations typically goes into the share-config-custom.xml file. The following is an example of stuff that you can configure in this file:
...
|
|Deployment - App Server|tomcat/shared/classes/alfresco/web-extension/share-config-custom.xml - Share configuration related to the whole Share application, such as Web Framework configuration and repository location (Remote) configuration. (Untouched by re-deployments and upgrades)|
|[Deployment All-in-One SDK project]({% link content-services/5.2/develop/sdk.md %}#getting-started-with-alfresco-content-services-sdk-3).|-   aio/share-jar/src/main/resources/META-INF/share-config-custom.xml - Share configuration related to a specific Share module extension, such as Document Library Action configuration, form configuration, aspect and type configuration.

|
|More Information|-   [Share configuration introduction]({% link content-services/5.2/develop/share-ext-points/share-config.md %}#configuring-alfresco-share)
-   [Document Library configuration]({% link content-services/5.2/develop/share-ext-points/share-config.md %}#alfresco-share-document-library-web-tier)

|
|Tutorials|-   [Making custom types visible]({% link content-services/5.2/tutorial/platform/content-model.md %}#share-configuration-for-custom-types-and-properties)
-   [Making custom aspects visible]({% link content-services/5.2/tutorial/platform/content-model.md %}#adding-an-aspect-with-share-configurations)
-   [Controlling search results]({% link content-services/5.2/admin/search.md %}#controlling-search-results)
-   [Controlling user name and password length](#configuring-share-user-name-and-password)
-   [Setting repository location](#configuring-the-share-default-port) (If the Share webapp and the repository webapp runs in different application servers)
-   [Remove persistent cookies](#configuring-share-to-remove-persistent-cookies)
-   [Document Library actions, metadata templates, and indicators](#override-and-extension-examples)
-   [Document Library views](#customizing-document-library-views)

|


## Configuring Alfresco Share {#configuring-alfresco-share}
...
You can customize Share through configuration files. Extending Share through more advanced techniques in discussed in the [Share extensions]({% link content-services/5.2/develop/software-architecture.md %}#share-extensions) section.
...
-   **[Share configuration files](#share-configuration-files)**  
Share can be configured through a number of configuration files.
-   **[Configuring Share user name and password](#configuring-share-user-name-and-password)**  
This information describes how to change the Share user name and password.
-   **[Configuring Share for mixed user name types](#configuring-share-for-mixed-user-name-types)**  
...
-   **[Configuring the Share default port](#configuring-the-share-default-port)**  
Use this information to configure the default port configuration for Share.
-   **[Configuring Share to remove persistent cookies](#configuring-share-to-remove-persistent-cookies)**  
Use this information to turn off cookies that store a user name after a session expires.
-   **[Configuring Share Actions with Smart Folders](#configuring-share-actions-with-smart-folders)**  
Share actions are disabled, by default, when using Smart Folders in Alfresco Content Services.
-   **[Enabling External Users panel](#enabling-external-users-panel)**  
...
-   **[Adding Custom MIME types](#adding-custom-mime-types)**  
...
The main Share configuration file is share-config.xml. This can be found in a default installation at tomcat/webapps/share/WEB-INF/classes/alfresco/share-config.xml. While it is possible to change configuration through direct changes to this file this is not recommended as any customizations will be lost if the Share WAR is re-exploded, or you install a new version of Alfresco Content Services.

To get around this issue it is advisable to make configuration changes to a file outside of the Share WAR. This can be done through the file share-config-custom.xml, which can be found at tomcat/shared/classes/alfresco/web-extension/share-config-custom.xml in the default Alfresco installation. Any changes made here will be applied once the changes have been saved, and Alfresco Content Services restarted. Further, your configuration changes can be saved between reinstalls and if the Share WAR re-explodes at any point your configuration file will be unaffected.
...
-   **[Configuring the Repository link](#configuring-the-repository-link)**  
...
-   **[Configuring aspects](#configuring-aspects)**  
Aspects can be configured in the file ./tomcat/webapps/share/WEB-INF/classes/alfresco/share-documentlibrary-config.xml.
-   **[Extending the Alfresco Share Document Library](#extending-the-alfresco-share-document-library)**  
...
-   **[Alfresco Share Document Library repository tier](#alfresco-share-document-library-repository-tier)**  
...
-   **[Alfresco Share Document Library web tier]({% link content-services/5.2/develop/share-ext-points/share-config.md %}#alfresco-share-document-library-web-tier)**  
...
-   **[Override and extension examples](#override-and-extension-examples)**  
...
-   **[Client-side template and action extensions](#client-side-template-and-action-extensions)**  
Two global events are available to make it easier to add new metadata template renderers and client-side action handlers.
-   **[Customizing document library views](#customizing-document-library-views)**  
...
-   **[Reference]({% link content-services/5.2/develop/reference/share-document-library-ref.md %}#reference)**  
...
## Alfresco Share Document Library web tier {#alfresco-share-document-library-web-tier}
...
|
|![]({% link content-services/images/status-indicators.png %})|-   Defined within the `DocumentLibrary` config section, status indicators are small icons typically used to indicate the presence of a marker aspect, or whether a document is in a particular state, for example checked out for editing.
...
A note about the `labelParam` `value`: refactoring on the client-side (JavaScript code) means that a common helper object is available for each node within the Document Library during the rendering cycle, namely `jsNode`. A full reference for this new resource is in [jsNode reference.]({% link content-services/5.2/develop/reference/share-document-library-ref.md %}#jsnode-reference)
...
![]({% link content-services/images/metadata-template.png %})
...
![]({% link content-services/images/doclib-actions.png %})
...
![]({% link content-services/images/doclib-metadata-template-renderer.png %})
...
See [EXIF renderer source code]({% link content-services/5.2/develop/reference/share-document-library-ref.md %}#exif-renderer-source-code) for the complete source for this example.
...
You can also create your own themes. Take a look at the [Adding a custom Share Theme]({% link content-services/5.2/tutorial/share/style.md %}#adding-a-custom-share-theme) tutorial.

-   **[Selecting Share themes](#selecting-share-themes)**  
...
-   **[Editing a Share theme](#editing-a-share-theme)**  
...
-   **[Use of forms in Share](#use-of-forms-in-share)**  
...
-   **[Customizing the validation handler](#customizing-the-validation-handler)**  
...
-   **[Displaying type metadata](#displaying-type-metadata)**  
...
-   **[Displaying aspect metadata](#displaying-aspect-metadata)**  
...
-   **[Changing the default set label](#changing-the-default-set-label)**  
...
-   **[Providing a custom form control](#providing-a-custom-form-control)**  
...
-   **[Changing the field label position](#changing-the-field-label-position)**  
By default, forms are rendered with the field labels positioned above the input control.
-   **[Providing a custom form template](#providing-a-custom-form-template)**  
...
![]({% link content-services/images/FormsMetadata_edit.png %})
...
![]({% link content-services/images/FormsArch.png %})
...
For a description of the available form controls, refer to [Forms reference]({% link content-services/5.2/develop/reference/share-document-library-ref.md %}#forms-reference).
...
## Configuring a form control {#configuring-a-form-control}
...
![]({% link content-services/images/Field_labels_on_top.png %})
...
4.  Save the file.
    ...
    ![]({% link content-services/images/Fields_labels_to_left.png %})
...
2.  When the configuration and template is in place, the Edit Metadata page for a `cm:content` type in Share has the following appearance.
    ...
    ![]({% link content-services/images/Fields_in_2_columns.png %})
