...
|Extension Point|Surf Pages (It is recommended to use [Aikau Pages]({% link content-services/5.2/develop/share-ext-points/aikau-pages.md %}#aikau-pages) instead)|
|---------------|--------------------------------------------------------------------------------------------------|
...
|Description|Surf pages are the "old school" pages that most of the Share UI is built up around. But more and more pages are converted to [Aikau pages]({% link content-services/5.2/develop/share-ext-points/aikau-pages.md %}#aikau-pages). All files involved in defining a Surf page are stored under /site-data and /templates.

Putting together a Surf page involves a lot of objects, such as page, template-instance, component, and so on. This is called the [siteData]({% link content-services/5.2/develop/reference/surf-framework-ref.md %}#surf-object-xml-quick-reference-sitedata) model and the following picture illustrates how these objects play together:

 ![]({% link content-services/images/dev-extensions-share-surf-page-model.png %}) The definition of a [Surf page]({% link content-services/5.2/develop/reference/surf-framework-ref.md %}#page) is done in XML and looks like this in a Hello World example:
...
The page definition refers to a [template instance]({% link content-services/5.2/develop/reference/surf-framework-ref.md %}#template-instance) that links to the physical template, it is defined in XML:
...
The page is built up of different regions, and each `region` is defined for a specific `scope`, such as `global` or `page`. If the scope is page then we always need to implement the rendition for the region. This is done via a [component]({% link content-services/5.2/develop/reference/surf-framework-ref.md %}#component), which is defined in XML:
...
A Surf page is processed and generated via the Spring MVC framework. The following picture gives an overview of how it works:
 ...
 ![]({% link content-services/images/dev-extensions-share-surf-pages-architecture.png %})
...
-   tomcat/shared/classes/alfresco/web-extension/templates - FreeMarker template location
 ...
 (These paths are untouched by re-depolyments and upgrades) For component Web Script file locations see: [Web Script]({% link content-services/5.2/develop/repo-ext-points/web-scripts.md %}#surf-web-scripts) section.

|
|[Deployment All-in-One SDK project]({% link content-services/5.2/develop/sdk.md %}#getting-started-with-alfresco-content-services-sdk-3).|-   aio/share-jar/src/main/resources/alfresco/web-extension/site-data/pages/
...
-   aio/share-jar/src/main/resources/alfresco/web-extension/templates
 ...
 For component web script file locations see: [Web Script]({% link content-services/5.2/develop/repo-ext-points/web-scripts.md %}#surf-web-scripts) section.|
|More Information|-   [Introduction to Surf Pages]({% link content-services/5.2/develop/software-architecture.md %}#introduction-to-surf-pages)) - This page contains a walk-through on how to create a Surf page, and it also has links to a page that shows how to create the same page with Aikau.
-   [Spring Surf Framework Guide]({% link content-services/5.2/develop/software-architecture.md %}#surf-framework-guide) - Deep dive into the Surf framework
-   [Site Date Model Reference]({% link content-services/5.2/develop/reference/surf-framework-ref.md %}#surf-object-xml-quick-reference-sitedata) - Details around all the model objects such as page, component, and template-instance.
-   [Surf root objects]({% link content-services/5.2/develop/api-reference.md %}#surf-root-objects)
-   [Component rendering root objects]({% link content-services/5.2/develop/api-reference.md %}#components)
-   [Template rendering root objects]({% link content-services/5.2/develop/alfresco-full-text-search-ref.md %}#templates)
...
|
|Tutorials|-   [Adding a new Surf page.]({% link content-services/5.2/tutorial/share/pages.md %}#adding-a-new-surf-page-to-share)
-   [Making a new Surf page the default page.]({% link content-services/5.2/tutorial/share/pages.md %}#making-the-new-page-the-default)
-   [Adding content to a Surf page]({% link content-services/5.2/tutorial/share/pages.md %}#adding-content-to-a-surf-page)
...
-   [Exploring root objects that can be used in a template.]({% link content-services/5.2/develop/repo-ext-points/web-scripts.md %}#exploring-root-objects)
...
In the [widget instantiation customization tutorial]({% link content-services/5.2/tutorial/share/doclib.md %}#customizing-a-surf-javascript-widget-in-the-document-library) the documentlist.get.html.ftl and documentlist.get.js files were modified to instantiate a custom JavaScript widget that extends the default `Alfresco.DocumentList`. This is a fragment from the webview.get.html.ftl file:
