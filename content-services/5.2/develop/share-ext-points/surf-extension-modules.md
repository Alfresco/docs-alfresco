...
|Description|Surf Extension Modules are the main tool to use when adding, updating, or hiding content in the Share User Interface (UI). They can be deployed and un-deployed during runtime. A module is defined in XML and stored in the site-data/extensions directory.
 ...
 Working with these extension modules assume a certain knowledge of the Surf UI development framework. Make sure that you have read through the [Share Architecture]({% link content-services/5.2/develop/software-architecture.md %}#share-architecture) section and the [Surf deep dive]({% link content-services/5.2/develop/software-architecture.md %}#surf-framework-guide) section.
...
|
|Deployment - App Server|tomcat/shared/classes/alfresco/web-extension/site-data/extensions (Untouched by re-deployments and upgrades)|
|[Deployment All-in-One SDK project]({% link content-services/5.2/develop/sdk.md %}#getting-started-with-alfresco-content-services-sdk-3).|aio/share-jar/src/main/resources/alfresco/web-extension/site-data/extensions|
|More Information|For a comprehensive introduction to Extension Modules see [this page](#introduction-to-surf-extension-modules), which also compares Share configuration with Extension Modules.|
|Tutorials|-   [Adding content to a Surf page]({% link content-services/5.2/tutorial/share/pages.md %}#adding-content-to-a-surf-page)
-   [Removing/hiding content from a Surf page]({% link content-services/5.2/tutorial/share/pages.md %}#removing-content-from-a-surf-page)
-   [Conditionally controlling rendering of content on Surf page]({% link content-services/5.2/tutorial/share/rendering.md %}#sub-component-evaluations)
-   [Customizing (Web Script Properties) the footer text for a Surf page]({% link content-services/5.2/tutorial/share/pages.md %}#customizing-web-script-properties) the footer text for a Surf page)
-   [Customizing (Web Script Controller) the WebView dashlet on the Dashboard page]({% link content-services/5.2/tutorial/share/pages.md %}#customizing-web-script-controller)
-   [Customizing (Web Script Template) the footer text for a Surf page]({% link content-services/5.2/tutorial/share/pages.md %}#customizing-web-script-template) the footer text for a Surf page)
-   [Extend an out-of-the-box Surf Widget (YUI)]({% link content-services/5.2/tutorial/share/doclib.md %}#customizing-a-surf-javascript-widget-in-the-document-library)
-   [Add a new menu item to "Create..." menu in DocLib]({% link content-services/5.2/tutorial/share/doclib.md %}#adding-a-menu-item-to-the-"Create..."-menu-in-doclib)
-   [Customizing the Share Header Style (Aikau)]({% link content-services/5.2/tutorial/share/header.md %}#customizing-the-share-header-style-aikau)
-   [Adding JS packages (Aikau)]({% link content-services/5.2/tutorial/share/amd.md %}#adding-amd-packages-via-extension-modules-aikau)

|

-   **[Introduction to Surf Extension Modules](#introduction-to-surf-extension-modules)**  
...
-   **[Module deployment and evaluators](#module-deployment-and-evaluators)**  
...
-   **[Module dynamic configuration](#module-dynamic-configuration)**  
...
If you look in the tomcat/webapps/share/WEB-INF/classes/alfresco directory of your Alfresco Content Services installation, then you’ll notice a number of files ending with -config.xml, such as for example share-config.xml. These files contain configuration that is loaded into a Spring bean when Alfresco Content Services starts and is accessed by Share code to dictate many different aspects of its behavior.
...
Each module is processed for every request that comes into the Share web application to determine what configuration that should be applied. If a module should only be processed for certain requests, then [evaluators]({% link content-services/5.2/develop/share-ext-points/evaluators.md %}#evaluators) can be used to determine when a module should be processed. This is an improvement to how the share-config-custom.xml configuration works, which is applied to all requests and you cannot add any evaluators.
...
![]({% link content-services/images/dev-extensions-share-module-sample-hide-external-user-invite.png %})

To hide this component, find out the `region-id`, `source-id`, and `scope` so it can be used when defining the extension module. If you are not up to speed on Surf see [Spring Surf deep dive]({% link content-services/5.2/develop/software-architecture.md %}#surf-framework-guide). You can find this information by using a tool called SurfBug ([more info on how to enable this tool]({% link content-services/5.2/develop/tools.md %}#introducing-surfbug)). When this tool is enabled, after refreshing the page, red lines will show up and mark the different components on the page:

![]({% link content-services/images/dev-extensions-share-module-sample-hide-external-user-invite-surfbug.png %})

Clicking the component area brings up an information window with all the Surf data that we need:

![]({% link content-services/images/dev-extensions-share-module-sample-hide-external-user-invite-surfbug-props.png %})
...
For more information about this module configuration see [controlling rendering of components]({% link content-services/5.2/tutorial/share/pages.md %}#removing-content-from-a-surf-page).
...
![]({% link content-services/images/dev-extension-share-module-deployment-screenshot-1.png %})
...
-   **[Auto-deploying modules](#auto-deploying-modules)**  
