...
-   `protected WritableByteChannel getDirectWritableChannel()` - should create a temp file that the currently uploaded file can be stored in.
 ...
 The content writer implementation does not actually write/save the file to the new storage. This is handled via a content stream listener that is registered via the content writer implementation. The custom content stream listener, such as the `DbContentStreamListener`, should implement the `org.alfresco.service.cmr.repository.ContentStreamListener` interface. Then implement the following methods:
...
|
|File Storage Buckets|The default behavior of the FileContentStore is to store content files in a content URL with the following URL format:
...
|[Deployment All-in-One SDK project]({% link content-services/5.2/develop/sdk.md %}#getting-started-with-alfresco-content-services-sdk-3).|-   aio/platform-jar/src/main/java/{domain specific package} - all the Java classes making up the custom content store implementation.
...
|
|More Information|-   [Using and configuring Content Stores]({% link content-services/5.2/admin/content-stores.md %}#setting-up-content-stores) - Main documentation page for out-of-the-box content stores
