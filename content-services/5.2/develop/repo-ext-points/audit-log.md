...
|Description|The Audit service can be used to keep a record (log) of all operations performed in the repository. The audit information is stored in a database in a form that is designed to be simple for third-party reporting tools to consume. The service allows auditing of virtually any system event, including metadata changes.
...
Further information can be found in the [Auditing Guide]({% link content-services/5.2/admin/audit.md %}#auditing).
...
```
 ...
 The audit configuration file starts with a definition of the Data Extractors that we will be using in our audit application. These components are present to take input data, such as a `NodeRef` object, and provide output data, such as a node name. For further information about Data Extractors [see this page]({% link content-services/5.2/admin/audit.md %}#overview).
...
|More Information|-   [Auditing Alfresco]({% link content-services/5.2/admin/audit.md %}#auditing)|
|Sample Code|-   [Sample audit log implementation](https://github.com/Alfresco/alfresco-sdk-samples/tree/alfresco-51/all-in-one/custom-audit-app-repo)|
