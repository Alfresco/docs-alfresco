---
title: Content Transformers (and Renditions)
---
...
-   Mediawiki markup **to** HTML
 ...
 When working with transformations and renditions it is important to make sure that the involved mimetypes are known to Alfresco Content Services. So when accessing the "mimetypes" Web Script make sure the mimetypes that will be used in transformations and renditions are included there, if not you would have to register them with Alfresco Content Services, see the [Mimetypes extension point]({% link content-services/5.2/develop/repo-ext-points/mimetypes.md %}) for more information about that.
...
```
 ...
 ![]({% link content-services/images/dev-extensions-repo-transformations-json2html-result.png %})
...
```
 ...
 If the solution being implemented is very transformation intensive a remote, then use the [Document Transformation Server]({% link content-services/5.2/install/index.md %}#document-transformation-engine-overview). It would be totally dedicated to performing transformations, and can be scaled out separately from the rest of the Alfresco Content Services system, depending on transformation load.
...
|
|[Deployment All-in-One SDK project]({% link content-services/5.2/develop/sdk.md %}#getting-started-with-alfresco-content-services-sdk-3).|-   aio/platform-jar/src/main/resources/alfresco/module/platform-jar/context/service-context.xml - Transformation bean definitions
...
-   [Invoking renditions from JavaScript]({% link content-services/5.2/develop/api-reference.md %}#rendition-service)
...
|
|Tutorials|-   [Extending the RuntimeExecutableContentTransformer]({% link content-services/5.2/tutorial/platform/web-scripts.md %}#extending-runtimeexecutablecontenttransformer) (note: this tutorial sets the `explicitTransformations` property, this is not necessary)
