...
approach described below. If you are using the [Alfresco SDK]({% link content-services/5.2/develop/sdk.md %})
you would want to follow the second approach.

### Installing the WAR file into an ACS Trial environment

1.  Stop the Alfresco server 
    
2.  Copy the WAR file, for example `api-explorer-5.2-***.war` into the `<alfresco-install-dir>/tomcat/webapps` directory. Then rename the file to `api-explorer.war`.

3.  Start the Alfresco server, this will deploy the API Explorer webapp

### Installing the WAR file into an Alfresco SDK AIO project

The API Explorer should be enabled by default.

If it is not available look in the `<project-dir>/pom.xml` and look for this section:
