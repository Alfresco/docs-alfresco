...
You can automatically replicate folders and content between repositories using replication jobs. These jobs are controlled by the replication service, which finds content that needs to be replicated and then calls the transfer service to carry out the replication. Replication suits an environment where you're running multiple, separate instances of Community Edition and then replicating a subset of the content between these servers.
...
You can configure Community Edition to replicate content between source and target repositories.
...
7. Configure the Alfresco Share URL to allow access to the source repository.
