...
This information describes the process for backing up the content repository only. It assumes that components other than the data residing in Community Edition (operating system, database, JRE, application server, binaries and configuration, etc.) are being backed up independently.

Your backup strategy must be tested end-to-end, including restoration of backups. Ensure that you've adequately tested your backup scripts prior to deploying Community Edition to production.

## Back up and restore repository

Backing up a repository involves backing up the directory pointed to by the `dir.root` setting, the database that Community Edition is configured to use, and the Solr indexes.
...
The `dir.root` directory is defined in the `alfresco-global.properties` file. By default, this directory is named `alf_data` and is located within the directory where Community Edition is installed.
...
1. Stop Community Edition.

2. Back up the database Community Edition is configured to use, using your database vendor's backup tools.
...
2. Backup the database Community Edition is configured to use, using your database vendor's backup tools.
...
Backup the following subdirectories of the Community Edition `dir.root` directory using whatever tools you're comfortable with (`rsync`, `xcopy`):

* `contentstore`
* `contentstore.deleted` (optional)
* `solr6Backup`

> **Note:** Do not attempt to backup the solr6/index subdirectory while Community Edition is running. This will cause Solr index corruption. Use `solr6Backup` instead.
