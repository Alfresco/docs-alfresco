...
API for talking to ECM systems. Community Edition provides the leading open-source CMIS implementation, as well 
...
WEB-INF directory or inside the classpath. Users can also manage these files inside the Community Edition server, 
...
vault (where your credentials are stored in an Community Edition-managed file).
...
Community Edition has a similar REST-based means for authenticating. It is slightly different in that the 
RESTful parameters are not the same as those of MediaWiki. Also, Community Edition hands back a ticket in an 
XML return payload. This ticket must be applied to the HTTP headers of every subsequent call so that Community Edition 
...
to connect to an Community Edition instance. All you need to do is define an endpoint that points to an 
Community Edition instance and uses the `alfresco` connector. Community Edition connectors use an 
...
Community Edition instance and access its remote API. For example, you can interact with the CMIS API on the 
...
`alfresco/site-data` directory. Usually core Community Edition objects would be located in `alfresco/site-data` 
...
Checksums can be applied to JavaScript and CSS resources to facilitate efficient upgrading of Community Edition.

One of the problems that has affected upgrades of Community Edition in the past is that the end-user's browser 
...
is disabled in Surf and is unlikely to ever enabled by default in future releases of Community Edition.
