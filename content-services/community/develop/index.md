...
[Software Architecture]({% link content-services/community/develop/software-architecture.md %}) section, as this will
...
There are two types of [extensions]({% link content-services/community/develop/overview-ext-points.md %}) that can be built: in-process extensions and out-of-process extensions.
...
You can see why developing out-of-process extensions is the way to go. See [SDK 5]({% link content-services/community/develop/oop-sdk.md %}) for more information. 
...
through the [Content model extension point]({% link content-services/community/develop/repo-ext-points/content-model.md %}) 
documentation and then walk through the [content model tutorial]({% link content-services/community/tutorial/platform/content-model.md %}). 
By implementing a content model you will get familiar with the in-process extension model and the associated 
[SDK 4]({% link content-services/community/develop/sdk.md %}).

When you have the custom content model in place, providing types and aspects related to the project domain (i.e. finance,
healthcare, manufacturing etc.), then you can move on and implement the business logic using [SDK 5 event handling and ReST API libraries]({% link content-services/community/develop/oop-sdk.md %}) 
