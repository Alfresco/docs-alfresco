...
The Event Gateway is a brand new Alfresco Component, introduced to improve the security of the [event mechanism]({% link content-services/community/develop/oop-ext-points/events.md %}). 
...
* (**Enterprise Edition Only**) Events are filtered by ACL, so users can only receive events of nodes they have permission to read.
...
For more information on how to use the subscription API see the [SDK 5 documentation]({% link content-services/community/develop/oop-sdk.md %}#using-event-gateway).
