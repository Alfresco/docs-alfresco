...
A patch is a piece of Java code that executes once when Community Edition starts. Custom patches can be implemented.
...
A patch executes a piece of Java code when Community Edition starts up, and logs the result in the 
`ALF_APPLIED_PATCHES` database table. A patch is only executed once and can be targeted at a certain Community Edition 
version range. Patches are used a lot by Community Edition internally to do things like database schema updates 
...
version of Community Edition for which it should be run. In the `bootstrap-context.xml` file define a bean as follows:
...
Community Edition version, so setting the `fixesToSchema` value to `${version.schema}`. Note that patches are 
...
Restarting Community Edition will not re-run the patch as it only runs once. You can check the status of a patch 
...
Community Edition version it was applied. If the patch failed then an error message is available. 
...
The patch will now be executed again if Community Edition is restarted. However, note that you would have to 
...
* See the [Bootstrap content]({% link content-services/community/develop/repo-ext-points/bootstrap-content.md %}) extension point for out-of-the-box patch implementations that import content.
