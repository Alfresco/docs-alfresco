...
A `ModuleComponent` executes code and is tied to a specific [Module]({% link content-services/community/develop/extension-packaging.md %}). 
...
A Community Edition module, such as an AMP or a JAR, can have tightly coupled components (that is, Java classes) 
...
A component can also be associated with a Community Edition version range for which it is valid, which means 
that it will only be invoked if current Community Edition version is in this version range. By default each one 
...
used to indicate for what Community Edition versions this component should be activated.
...
[extension point]({% link content-services/community/develop/repo-ext-points/bootstrap-content.md %}).
