...
For each user profile we bootstrap, Community Edition also needs some internal account information set up, 
...
about Bcrypt support, see [Cryptographic password hashing]({% link content-services/community/admin/security.md %}#bcryptoverview).
...
defined within Community Edition and not synchronized from an external source, such as LDAP. The `APP.DEFAULT` 
...
content into the Repository, for more information about this see [Module Components]({% link content-services/community/develop/repo-ext-points/module-components.md %}).
...
on Community Edition startup. If the store does not yet exist (the very first time a new Community Edition 
installation starts up), the data will aways be bootstrapped, no matter the value, but from that moment on the store exists, 
and setting this to false means the next time Community Edition reboots these files will not be overridden with 
...
* [Bcrypt password encryption support]({% link content-services/community/admin/security.md %}#bcryptoverview)
