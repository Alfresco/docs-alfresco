...
to each other is defined using *Associations*. A typical association that comes out-of-the-box with Community Edition 
is the one between a folder node and its child nodes, this type of association is a `child-association`. So when you start 
using Community Edition you can immediately organize folders and files into a hierarchy, like with a file system.

The Types, Aspects, Properties, and Associations are in turn organized into models that we call *Content Models*. 
Community Edition comes with a number of Content Models out of the box for different things like general folder 
...
property with, for example, a data type integer? Community Edition also comes with a *Meta Model*. The Meta model 
...
Both the custom content model and the Community Edition content model is defined by the Data Dictionary Schema, 
...
[Admin Console]({% link content-services/community/admin/admin-console.md %}). At the end of this article you will find information about where 
...
|`mandatory`|element (boolean)|`[0..1`]|`true` = when a property is set as mandatory it tells Community Edition that the property is required. By default, this is not enforced. Instead, Community Edition marks content items with empty mandatory properties with the aspect `sys:incomplete`. This is done so that you can create items that have mandatory properties even if the value of the property is not known at the time of content creation, while still indicating that the property is required (eventually). Mandatory properties will have a `*` next to them in the UI.<br><br>`false` = it is optional and this is the default if this element is not specified.<br><br>The `mandatory` element can also have a `boolean` attribute called `enforced`. If this is set to `true` then you cannot create a node without this property having a value.|<br><br>
...
the aspects that comes with Community Edition out-of-the-box:
...
Community Edition APIs to create a node with a type from this content model, and one or more aspects applied 
...
>**Important:** It is possible to create custom content models from the Alfresco Share UI without the need to use XML. These models can then be exported as XML and included in a build project. See the [Model Manager in Share Admin Tools documentation]({% link content-services/community/config/models.md %}) for further information.
...
CMIS defines a data model, which encapsulates the core concepts found in most repositories. Community Edition 
provides an implementation of the CMIS bindings and maps the content metamodel to the CMIS domain model. This allows 
content models defined in Community Edition to be exposed and manipulated by using CMIS.
...
The Community Edition content metamodel is mapped to the CMIS data model as follows:
...
Use the **Model and Messages Console** in the [Repo Admin Console]({% link content-services/community/admin/admin-console.md %}) to manage models.
...
2.  In the **Consoles** section, click **Model and Messages Console**.
    ...
    > **Note:** The Admin Console options differ if you're using Community Edition.
...
* [Defining and Deploying a custom Content Model for Data Lists]({% link content-services/community/develop/repo-ext-points/data-lists.md %})
* [Displaying types]({% link content-services/community/develop/share-ext-points/share-config.md %}#displaytypemetadata) - more information about how to display type properties
* [Displaying aspects]({% link content-services/community/develop/share-ext-points/share-config.md %}#displayaspectmetadata) - more information about how to display aspect properties
* [Grouping fields in forms]({% link content-services/community/develop/share-ext-points/share-config.md %}#displayfieldsingroups)
* [Model Manager in Share Admin Tools]({% link content-services/community/config/models.md %})
...
* [Defining and Deploying a custom Content Model]({% link content-services/community/tutorial/platform/content-model.md %}) (Short introduction excluding localization)
