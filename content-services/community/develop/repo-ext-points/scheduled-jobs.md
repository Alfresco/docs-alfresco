...
Community Edition automatically runs a number of scheduled jobs, for example the content store cleaner job and 
...
A scheduled job in Community Edition can be compared to a Unix [cron job](https://en.wikipedia.org/wiki/Cron){:target="_blank"}. 
...
the job. The start delay is important as this makes it possible to delay all scheduled jobs until the Community Edition 
...
to be in the form of a [Repository Action]({% link content-services/community/develop/repo-ext-points/repo-actions.md %}). Having the job implemented as a repository 
...
actions [here]({% link content-services/community/develop/repo-ext-points/repo-actions.md %}). We use the `ServiceRegistry` to get to the 
