...
Here you will notice that all the Community Edition-related JavaScript files have big numbers in their file names. 
...
on a Surf page. If you are not up to speed on Surf, see [Surf framework]({% link content-services/community/develop/software-architecture.md %}#surf-framework).
...
* If you are running the Community Edition server locally, and are using the default port for Tomcat servers, then the URL would be `http://localhost:8080/share/page/surfBugStatus`.
