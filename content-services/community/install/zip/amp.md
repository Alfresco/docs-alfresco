...
The MMT is available as a JAR file from the distribution zip (`alfresco-content-services-community-distribution-6.2.0-ga.zip`), in the zip's `/bin` directory. Place the `/bin` directory and its contents in the same location where you extracted the Community Edition distribution zip; for example `<installLocation>/bin`.
