...
1.  Add a new Surf Extension Module file and define the Send-As-Email action
    ...
    |`icon`|Share looks for an icon that starts with this name and ends with `-16.png`. So it will look for `email-16.png` in our case. Community Edition expects the image file to be located in the `/components/documentlibrary/actions` directory. If not set, the `id` is used.|
