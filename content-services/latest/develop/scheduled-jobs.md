...
to be in the form of a [Repository Action]({% link content-services/latest/develop/repo-actions.md %}). Having the job implemented as a repository 
...
actions [here]({% link content-services/latest/develop/repo-actions.md %}). We use the `ServiceRegistry` to get to the 
