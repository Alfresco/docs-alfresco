...
* Ensure that you have a functional [backup of your Alfresco repository and database]({% link content-services/7.1/admin/backup-restore.md %}), before starting the upgrade process.
* Download and run the [Alfresco Extension Inspector]({% link content-services/7.1/develop/extension-inspector.md %}) to understand which customization or library items need to be reviewed or updated to support the upgrade.
...
* Content Services 7.1 introduces changes that require new releases of some modules. To upgrade to 7.1, you also need to update any of the module artifacts to which you're entitled. See [Supported platforms]({% link content-services/7.1/support/index.md %}) for more details on the associated versions.
...
Use this procedure to upgrade from a previous version of Content Services using one of the supported [installation methods]({% link content-services/7.1/install/index.md %}). The process involves a new installation of the Content Services binaries and configuration, and an in-place upgrade of a copy of the repository.
...
1. Install the new version of Content Services.
    ...
    2. [Back up your existing repository]({% link content-services/7.1/admin/backup-restore.md %}) (`alfresco-v.1`) and the database.
...
2. Validate the new 7.1 installation to check that it's working correctly.
    ...
    2. [Start the server]({% link content-services/7.1/install/zip/additions.md %}#start-server) and [validate](#validate-upgrade) that the system works correctly.

3. Apply all customizations to the new 7.1 installation.

    1. [Stop]({% link content-services/7.1/install/zip/additions.md %}#stop-server) the server.

    2. [Remove]({% link content-services/7.1/install/zip/tomcat.md %}#tailor-your-installation) any unwanted applications.

    3. [Modify]({% link content-services/7.1/config/index.md %}#customize-applications) applications.

    4. Install the required AMP files. See [Installing an Alfresco Module Package]({% link content-services/7.1/install/zip/amp.md %}).
    ...
    6. [Start]({% link content-services/7.1/install/zip/additions.md %}#start-server) the server.
    ...
    8. [Stop]({% link content-services/7.1/install/zip/additions.md %}#stop-server) the server.
...
4. Restore production data.
    ...
    4. [Start]({% link content-services/7.1/install/zip/additions.md %}#start-server) the server.
...
* Check if all the dashboards created previously still exist.
* Check if the folders in the document library that were created prior to the upgrade still exist.
* Check if clustering is working properly by running the [cluster validation tool]({% link content-services/7.1/admin/cluster.md %}#managecluster) in the Admin Console.
...
>**Note.** This is only applicable if upgrading from 6.2 or earlier versions. If upgrading from version 7.0, then the
>patch should already be applied.
