...
1. Download and install Tomcat following the instructions from [http://tomcat.apache.org](https://tomcat.apache.org){:target="_blank"}.
    ...
    See the [supported platforms]({% link content-services/7.1/support/index.md %}) page for the supported versions.
...
You're now ready to [install any additional software]({% link content-services/7.1/install/zip/additions.md %}) that you require, and [install integrations]({% link content-services/7.1/install/zip/additions.md %}#install-integrations).
...
[JAVA parameters]({% link content-services/7.1/admin/security.md %}#alfresco-keystore-configuration).

1. Move the default keystore files to your installation

    1. Extract from the `alfresco-content-services-distribution-7.1.x.zip` file the following two files:
...
Next, you can [customize applications]({% link content-services/7.1/config/index.md %}#customize-applications) such as the configuration for Content Services, Alfresco Share, and Alfresco Search Services.

### Adding a reverse proxy in front of Content Services

See [adding a reverse proxy]({% link content-services/7.1/admin/securing-install.md %}#addreverseproxy)
