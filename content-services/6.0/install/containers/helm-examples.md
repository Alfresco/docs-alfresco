...
Follow the [EKS deployment]({% link content-services/6.0/install/containers/helm.md %}#helm-deployment-with-aws-eks) guide up to the [Choose Content Services version]({% link content-services/6.0/install/containers/helm.md %}#choose-content-services-version) section. Return to this page once the Docker registry secret is installed.
...
* [Configure email]({% link content-services/6.0/config/email.md %})
...
The Alfresco content repository Docker image comes with a pre-created default keystore that contains a secret key. For more information, see [Managing Alfresco keystores]({% link content-services/6.0/admin/security.md %}#managealfkeystores) and [Dockerfile](https://github.com/Alfresco/acs-packaging/blob/master/docker-alfresco/Dockerfile#L81-L85){:target="_blank"}.
...
Otherwise, refer to the full list of configuration options in [Keystore configuration]({% link content-services/6.0/admin/security.md %}#keystore-configuration).
