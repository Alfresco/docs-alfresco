...
1. Launch the [Admin Console]({% link content-services/7.2/admin/admin-console.md %}#launchadminconsole).
...
3. Update `share-config-custom.xml` as described in [Configuring the Share default port]({% link content-services/7.2/develop/share-ext-points/share-config.md %}#setting-default-port).
...
* You've already set up Content Service with Tomcat 9, running HTTP on port 8080.
  * Follow the steps in [Install using distribution zip]({% link content-services/7.2/install/zip/index.md %}) if you haven't already done so.
...
1. Copy the `alf_data/keystore` folder from the distribution zip to `<CATALINA_BASE>/alf_data/keystore`.
    ...
    See [Install Alfresco WARs]({% link content-services/7.2/install/zip/tomcat.md %}) to review the structure of the distribution zip.
...
For more information see [metadata extraction extension point]({% link content-services/7.2/develop/repo-ext-points/metadata-extractors.md %}).
...
See [Managing aspects]({% link content-services/7.2/using/content/files-folders.md %}#applyaspects) for more information.
...
2. Locate and edit the `alfresco-global.properties` file in the `<classpathRoot>` directory.
    ...
    See [Modifying the global properties file]({% link content-services/7.2/config/index.md %}#modify-global-props) for more information.
...
> **Note:** This section only applied from Alfresco Content Services 7.2.1.3 onwards.
