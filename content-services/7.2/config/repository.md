...
| scripts.execution.maxMemoryUsedInBytes | The maximum memory (in bytes) a script is allowed to use. If script execution exceeds the configured memory, it will be stopped. <br><br>To enable this limit, set the property with a value bigger than zero. The default value is  `-1` (disabled). If you would like to use this setting, 10000000 bytes (10 MB) is a reasonable value for custom scripts. <br><br>This configuration only works with the supported JVM. |
