...
The search string syntax depends on the given query language and can differ significantly between AFTS and Lucene. However, there are some shared aspects that provide the same behavior in both languages, for more see [Field queries]({% link search-enterprise/3.0/using/field.md %}).
