...
Process Services can be configured to authenticate using the [Identity Service]({% link identity-service/1.1/index.md %}).
...
The Identity Service needs to be [deployed]({% link identity-service/1.1/install/index.md %}) and [configured]({% link identity-service/1.1/config/index.md %}) with an identity provider before being set up with other Alfresco products.
