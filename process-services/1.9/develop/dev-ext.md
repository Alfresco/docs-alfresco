...
Before beginning, you should read the [Installing]({% link process-services/1.9/install/index.md %}) and [Configuring]({% link process-services/1.9/config/index.md %}) sections to make sure you have an understanding of how Process Services is installed and configured.
...
See [Event processing for analytics]({% link process-services/1.9/config/external.md %}#event-processing-for-analytics) for details on how to configure events to be sent to Elasticsearch.
...
You can set up an external instance of ElasticSearch on your machine and configure Process Services to connect to it.

It is possible to [configure Elasticsearch in embedded mode]({% link process-services/1.9/config/external.md %}#embedded)

The following section describes the steps in more detail.

1. Download and set up ElasticSearch

    ElasticSearch provides downloads in multiple formats, however the ZIP download is recommended for general development usage. You must use ElasticSearch 1.7.3 with the Java client embedded in Process Services. The same can be downloaded from the [ElasticsSearch releases archive](https://www.elastic.co/downloads/past-releases/elasticsearch-1-7-4){:target="_blank"}.
...
2. Setting up Sense
    ...
    You must install the latest version of [Kibana](https://www.elastic.co/products/kibana){:target="_blank"} in order to be able to use the Sense plugin. This version of Kibana supports ElasticSearch 1.7.x but in order to use it you must add the following lines to the end of your `config/kibana.yml` file in order to turn off the core Kibana functionality which will not work with older versions of ElasticSearch. This does not matter, since the Sense plugin runs standalone and connects to ElasticSearch directly.
...
See [External Identity Management]({% link process-services/1.9/config/authenticate.md %}#ldap-and-active-directory) for more information on how to set this up. The basic idea behind it is that the LDAP server will periodically be polled and the IDM data in the database tables will be synchronized: created, updated or deleted depending on what the LDAP server returns and what currently is in the database tables.
