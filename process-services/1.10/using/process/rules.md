...
In the following, we will create a simple process that makes use of a Decision task and its Decision Table. We will use the [BPMN editor]({% link process-services/1.10/using/process/bpmn.md %}), but you can just as well use the [Step editor]({% link process-services/1.10/using/process/step.md %}) to achieve the same result.
...
The process' start form is shown below and defines 4 fields: `obligationsCompleted` (boolean), `additionalAchievements` (string), `completedDate` (date) and `dueDate` (date). See the [Form editor]({% link process-services/1.10/using/process/form.md %}) section for more information on how to create forms.
