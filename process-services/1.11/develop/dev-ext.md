...
Before beginning, you should read the [Installing]({% link process-services/1.11/install/index.md %}) and [Configuring]({% link process-services/1.11/config/index.md %}) sections to make sure you have an understanding of how Process Services is installed and configured.
...
See [Event processing for analytics]({% link process-services/1.11/config/external.md %}#event-processing-for-analytics) for details on how to configure events to be sent to Elasticsearch.
...
2. Bean implementation
   ...
   import java.util.Map;
    ...
    public class CustomVariablesReportGenerator extends AbstractReportGenerator {
      ...
      @Override
      public ReportDataRepresentation generateReportData(ProcessEngine processEngine, AnalyticsClient analyticsClient,
                                                         ReportingIndexManager indexManager, UserCache userCache,
...
3. Implementing `generateReportData()`
    ...
    * Populate chart/table data from the query results
    ...
    A protected helper method `executeQuery()` is provided which provides a concise syntax to execute an ElasticSearch search query given a query and optional aggregation, the implementation of which also provides logging of the query generated by the Java client API before it is sent. This can help with debugging your queries using Sense, or assist you in working out why the Java client is not generating the query you expect.

    ```java
    executeQuery(analyticsClient, indexManager, currentUser, status)
    ```

    The log4j configuration required to log queries being sent to ElasticSearch via `executeQuery()` is as follows

    ```text
    log4j.logger.com.activiti.service.reporting.AbstractReportGenerator=DEBUG
    ```

   Alternatively you can manually execute any custom query directly via the `AnalyticsClient` instance passed to the `generateReportData()` method, for example:

    ```java
    SearchSourceBuilder baseQuery = createBaseQuery(status);
    SearchRequest searchRequest = new SearchRequest(indexManager.getIndexForUser(currentUser, ElasticSearchConstants.INDEX_PROCESS_INSTANCES));
    searchRequest.source(baseQuery);
    
   return analyticsClient.search(searchRequest);
    ```

    Generating chart data from queries can be accomplished easily using the converters in the `com.activiti.service.reporting.converter` package. This avoids the need to iterate over returned query results in order to populate chart data items.

    Initially two converters `AggsToSimpleChartBasicConverter` and `AggsToMultiSeriesChartConverter` are provided to populate data for pie charts (which take a single series of data) and bar charts (which take multiple series) respectively. These two classes are responsible for iterating over the structure of the ES data, while the member classes of `com.activiti.service.reporting.converter.BucketExtractors` are responsible for extracting an actual value from the buckets returned in the data.
...
See [External Identity Management]({% link process-services/1.11/config/authenticate.md %}#ldap-and-active-directory) for more information on how to set this up. The basic idea behind it is that the LDAP server will periodically be polled and the IDM data in the database tables will be synchronized: created, updated or deleted depending on what the LDAP server returns and what currently is in the database tables.
