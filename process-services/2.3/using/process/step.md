...
>**Note:** A user with administration privileges can upload global templates. An administrator can add templates on the Tenant page of the [Identity Management]({% link process-services/2.3/using/process/index.md %}#identity-management) app.
...
See the [Business rules - decision tables]({% link process-services/2.3/using/process/rules.md %}) section for more details on Decision Tables.
...
>**Note:** Alfresco Actions are asynchronous. This is important to note if you have multiple tasks executing against the same node(s) in Alfresco Content Services. To control a sequence of actions against a node, use a [Service Task]({% link process-services/2.3/using/process/bpmn.md %}#service-task) instead.
