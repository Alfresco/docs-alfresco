...
|keycloak.token-store| *Required.* The location of where the account information token is stored. Possible values are `cookie` or `session`. **Note:** If you have Process Services configured with the Identity Service you must set `keycloak.token-store=cookie` otherwise upon logging out of Process Services a user will be redirected to an error page for more see [Running Identity Service with Process Services]({% link identity-service/latest/config/index.md %}#running-identity-service-with-process-services) |
