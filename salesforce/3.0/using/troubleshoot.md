...
Edit the Alfresco connected app definition, as specified in [Step 3: Configure app in Salesforce]({% link salesforce/3.0/install/index.md %}#configappinsalesforce-lightning) and ensure that the lifecycle class is set to `AlfrescoCanvasLifeCycleHandler`.
...
Check that the correct Share URL is specified in the **Canvas App URL** in Salesforce. See [Step 3: Configure app in Salesforce]({% link salesforce/3.0/install/index.md %}#configappinsalesforce-lightning) for more guidance.
...
To resolve this issue, remove the canvas app from the Classic UI page layout and follow the instructions to [Set up the App for use in Lightning]({% link salesforce/3.0/install/index.md %}#addappusinglightningcomponent).
