...
7. Copy the AMP file you downloaded during [installation]({% link microsoft-azure/2.1/install/index.md %}) to the `amps` directory.
...
9. Check that the [configuration]({% link microsoft-azure/2.1/config/index.md %}) is set up correctly for your environment.
    ...
    > **Note:** To upgrade a system that's never used the file system (i.e. on-premises installation without locally saved binaries), we recommend that you choose a pure Azure content store. See [Azure Connector content store subsystems]({% link microsoft-azure/2.1/config/index.md %}#azure-subsystems) for more details.

10. Starting from version 1.2, the Azure Connector has the deleted content store disabled by default, since this feature is already present in Microsoft's Azure Storage services. For details on how to re-enable it, see [Azure Connector deleted content store]({% link microsoft-azure/2.1/config/index.md %}#azure-connector-deleted-content-store).
