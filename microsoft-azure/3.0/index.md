...
ability to accelerate the local download of content. See [Direct Access URLs]({% link microsoft-azure/3.0/config/direct-access.md %}) 
for more details. It also introduces [Cloud Storage Properties]({% link microsoft-azure/3.0/config/index.md %}#cloud-storage-properties).

Other features introduced in previous versions include:

* [Azure content store subsystems]({% link microsoft-azure/3.0/config/index.md %}#azure-subsystems)
* [Multiple Azure storage container configuration]({% link microsoft-azure/3.0/config/index.md %}#configuring-multiple-storage-containers)
...
> **Note:** The Alfresco Content Connector for Azure module can be applied to Content Services 7.1 or later.
