...
| connector.az.apiEndpoint | Allows configuring a different Azure endpoint. The default value is `blob.core.windows.net`. The value of this property is concatenated with the `connector.az.account.name` property in order to build the final URL, for example, `https://<connector.az.account.name>.<connector.az.apiEndpoint>`. |
