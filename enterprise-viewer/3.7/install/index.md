...
* `alfresco-enterprise-viewer-package-3.7.x.zip`

### Java

Enterprise Viewer requires Java 11 or above. Consult your repository of choice for more detailed requirements. If you are using Java 17, refer to our [Java 17 support guide]({% link enterprise-viewer/3.7/install/java-support.md %}).

### Alfresco repository version

See the [Supported Platforms]({% link enterprise-viewer/3.7/support/index.md %}) for more information.
...
| Option 1 | Deploy AEV to the Alfresco Tomcat. <br><br>Skip to the [OpenContent install]({% link enterprise-viewer/3.7/install/index.md %}#installoc) section since no proxy will need to be installed. |
...
When installing a proxy please note that you are not limited to using Apache or NGINX. These are just two common options which we cover example installs of below. As long as the above routes are proxied appropriately you can move onto the [AEV install]({% link enterprise-viewer/3.7/install/index.md %}#install).
...
> * `{Application Base URL}/oat` (if installed)
>
> Next, go to the [AEV install]({% link enterprise-viewer/3.7/install/index.md %}#install).
...
2. Copy the OpenContent AMP to the Alfresco Content Services installation:
   ...
   * If using Alfresco Content Services 7.2.x, use the `tsgrp-opencontent-3.7-for-acs7.2.amp`.
   * If using Alfresco Content Services 7.3.x, use the `tsgrp-opencontent-3.7-for-acs7.3.amp`.
   * If using Alfresco Content Services 7.4.x, use the `tsgrp-opencontent-3.7-for-acs7.4.amp`.
...
7. Update the provided `openannotate-override-placeholders.properties` file:
    ...
    * `collaborationEndpoint=http://${server}:${port}`
    ...
    Replace the `${server}` and `${port}` placeholders in the above URL with the correct server and port values for the environment being installed to (See the section [Install collaboration features]({% link enterprise-viewer/3.7/install/index.md %}#collab))
...
6. (Optional) This step is only required if using the Enterprise Viewer and leveraging the "Collaboration Server" functionality for collaborative annotation functionality:
   ...
   * `collaborationEndpoint=http://${server}:${port}`
   ...
   the environment being installed to. See the section [Install collaboration features]({% link enterprise-viewer/3.7/install/index.md %}#collab).
