...
Stamps with predefined images and subject text are fully supported without restrictions. Limited support is available for custom stamps not generated directly in AEV. To view non-AEV stamps, which are stamps that are not generated in AEV, you must set the configuration `excludeEmbeddedAnnotations` to `false`. For more information on `excludeEmbeddedAnnotations`, see [Enable Embedded Annotations]({%link C:/Users/mbujak/alfresco_projects/docs-alfresco/enterprise-viewer/3.7/config/files.md %}#excludeEmbeddedAnnotations).
