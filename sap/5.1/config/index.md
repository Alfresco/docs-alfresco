...
2. Enter mandatory values for the new repository.
    ...
    * Values in _italics_ are customer specific and need to be adapted according to your environment.
    ...
    | SSL Port Number | The port number for secure layer. Only required for [Communication via HTTPS]({% link sap/5.1/admin/reference.md %}#securecomms). <br>Value: For now, leave empty. |
