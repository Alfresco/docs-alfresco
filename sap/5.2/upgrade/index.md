...
Make sure you've fulfilled all prerequisites for the SAP Connector, as described in [Prerequisites]({% link sap/5.2/install/index.md %}#prerequisites) and [Supported Platforms]({% link sap/5.2/support/index.md %}).
...
6. Download the new SAP Connector {% include tooltip.html word="AMP" text="AMP" %} files from [Hyland Community](https://community.hyland.com/){:target="_blank"}.
    ...
    Follow the instructions in the [Install SAP Connector]({% link sap/5.2/install/index.md %}#installsapconnamps) page. After applying the {% include tooltip.html word="AMP" text="AMP" %} files, verify the new SAP Connector version:
...
The structure of the UI is similar to what was available in previous releases, with a few changes. See [SAP Connector Administration Console]({% link sap/5.2/admin/index.md %}#sapadminconsole) for more details.
...
Install the license via the **License Information** section. See [Installing the license]({% link sap/5.2/install/index.md %}#installing-the-license) for more.
...
To recreate the SAP Content Repositories and their certificates, follow the steps in [(1) Create SAP Content Repository]({% link sap/5.2/config/index.md %}#basic-createsapcontentrepo) starting from **step 3**. Make sure you follow each required step including the functional test.
...
If there's an advanced configuration for [Opening associated Business Object in SAP]({% link sap/5.2/config/advanced.md %}#openassocbusinessobjinsap), this must also be merged into the new SAP Connector structure in the **Data Dictionary**. In order to do so, follow these step-by-step instructions:
...
2. Identify all necessary files used to enhance the feature (see [Advanced configuration]({% link sap/5.2/config/advanced.md %}#OpenBusinessObjectSAPAdvancedConfig) for more info).
...
Perform the functional (technical) testing of each recreated SAP Content Repository, as described in the [Basic configuration]({% link sap/5.2/config/index.md %}#basic-configuration).
...
Delete the former *connexas* license, as it's invalid. The license is stored on the Content Services server, in the folder specified by the `dir.license.external` property in the `alfresco-global.properties`. See [Installing the license]({% link sap/5.2/install/index.md %}#installing-the-license) for more.
