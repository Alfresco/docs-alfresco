...
In these topics you'll install and set up everything you need to run the SAP Connector. See [Prerequisites]({% link sap/5.2/install/index.md %}#prerequisites) and [Supported Platforms]({% link sap/5.2/support/index.md %}) for information on what you require before you start the installation.
...
1. Go to [Hyland Community](https://community.hyland.com/products/alfresco){:target="_blank"}, click **Product downloads**, and then download the SAP Connector distribution zip, which contains the following files:
    ...
    * `sap-content-connector-repo-5.2.x.amp` for Content Services.
    * `sap-content-connector-share-5.2.x.amp` for Alfresco Share.
    ...
    * `sap-content-connector-encryptor-1.0.jar` to [encrypt plain-text passwords]({% link sap/5.2/admin/reference.md %}#encryptpwd) for all SAP Connector related properties in the `alfresco-global.properties` file.
...
1. Prepare the files for merge:
    ...
    1. Create a new temporary directory and copy the following files into it:
        ...
        * `sap-content-connector-repo-5.2.x.amp`
    ...
    2. Verify the `sap-content-connector-repo-5.2.x.amp` file size. It should be less than 1 MB.
...
3. Verify the merge:
    ...
    Verify that the merge is successful and double-check the file size of the `sap-content-connector-repo-5.2.x.amp` again. This should now be much larger than before.
...
4. Copy and save the re-packaged AMP file:
    ...
    The re-packaged `sap-content-connector-repo-5.2.x.amp` file should now be saved for the future. This is the final file that's needed to [install](#installsapconnamps) the SAP Connector.
...
4. Check that the [configuration]({% link sap/5.2/config/index.md %}) is set up correctly for your environment.
...
> **Note:** There are additional properties that can be used to login to the SAP system via the SAP JavaConnector (such as using Logon Groups instead of the Gateway). See [Additional SAP JCo properties]({% link sap/5.2/admin/reference.md %}#sapjavaconprops) which lists the additional properties that are supported.
...
| integrations.sap.system.1.al.alfrescoPassword | Password for the user. Either plain-text or use encrypted password. See [Encrypting passwords]({% link sap/5.2/admin/reference.md %}#encryptpwd) for more. <br>Example value: `H3ll0W0rlD112!` or `ENC(XbfE4Z112==)` |
...
| integrations.sap.system.1.password | Password for the SAP user. Either plain-text or use encrypted password. See [Encrypting passwords]({% link sap/5.2/admin/reference.md %}#encryptpwd) for more. <br>Example value: `H3ll0W0rlD112!` or `ENC(XbfE4Z112==)` |
...
1. Install the `sap-content-connector-repo-5.x.amp` for Content Services on each node in the cluster.
    ...
    >  **Important:** Make sure you only use the merged SAP Connector repository AMP file, which includes the native SAP libraries, as described in [Installing overview]({% link sap/5.2/install/index.md %}).
