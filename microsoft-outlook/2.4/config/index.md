...
4. In **Prevent email duplication in**, choose to check the uniqueness of files and at what level.
    ...
    >**Note:** If an email is dropped into a folder, where the same email document already exists, the version detection feature will recognize it, and the **Versioning** dialog is displayed. See [Managing file versions in Outlook]({% link microsoft-outlook/2.4/using/index.md %}#managing-file-versions-in-outlook) for more details.
...
4. Select Licenses from the Tools menu and click **Edit** to add new licenses.
    ...
    See [Installing server and client licenses in Alfresco Share]({% link microsoft-outlook/2.4/install/index.md %}#installserverclientlicenses) for more information about installing licenses.
...
The `AlfrescoClientSettings-2.4.x.xml` file contains advanced configuration properties.
Use this file to set up attributes and metadata settings.

1. Locate and open `AlfrescoClientSettings-2.4.x.xml` in the `C:\Users\<username>` directory, where `<username>` is your Windows user name.
...
2. Configure the attributes that you need for the base `<outlook>` element:
    ...
    |`visibleNodes`|Controls content visible in the Explore tab in the Alfresco sidebar|`default`: all files and folders are visible. This is the default setting.<br><br>`favdocument`: only files marked by the user as a favorite are visible<br><br>`favfolder`: only folders marked by the user as a favorite are visible<br><br>`favonly`: only files and folders marked by the user as a favorite are visible|
