...
4. In **Prevent email duplication in**, choose to check the uniqueness of files and at what level.
    ...
    >**Note:** If an email is dropped into a folder, where the same email document already exists, the version detection feature will recognize it, and the **Versioning** dialog is displayed. See [Managing file versions in Outlook]({% link microsoft-outlook/2.6/using/index.md %}#managing-file-versions-in-outlook) for more details.
...
4. Select Licenses from the Tools menu and click **Edit** to add new licenses.
    ...
    See [Installing server and client licenses in Alfresco Share]({% link microsoft-outlook/2.6/install/index.md %}#installserverclientlicenses) for more information about installing licenses.
...
The `AlfrescoClientSettings-2.6.x.xml` file contains advanced configuration properties.
Use this file to set up attributes and metadata settings.

1. Locate and open `AlfrescoClientSettings-2.6.x.xml` in the `C:\Users\<username>` directory, where `<username>` is your Windows user name.
