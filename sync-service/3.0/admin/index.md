...
1. Create a custom model from the **Model Manager** in Alfresco Share **Admin Tools**.
    ...
    Follow the steps in [Creating a new model]({% link content-services/6.0/config/models.md %}#create-a-content-model).
...
2. Click the model name to start creating an associated aspect.
    ...
    Follow the steps for [Creating new aspects]({% link content-services/6.0/config/models.md %}#custom-types-aspects-and-properties).
...
5. Create a folder rule that applies the aspect to newly created and updated content.
    ...
    Follow the steps in [Defining rules for a folder]({% link content-services/6.0/using/content/rules.md %}#defining-rules-for-a-folder).
    ...
    See [Folder rules]({% link content-services/6.0/using/content/rules.md %}) for more details.
...
> **Note:** Apply the new aspect to all existing content in the folder, and subfolders. Adding an aspect manually to existing folders doesn't cascade the extra functionality down the hierarchy. See [Applying aspects]({% link content-services/6.0/using/content/files-folders.md %}#applyaspects) for more details.
...
1. To perform a backup of your Sync Service database, follow these steps:
    ...
    1. Backup your repository database. See [Back up and restore]({% link content-services/6.0/admin/backup-restore.md %}).
...
2. To perform a restore, follow these steps:
    ...
    2. Restore the repository database. See [Back up and restore the repository]({% link content-services/6.0/admin/backup-restore.md %}).
