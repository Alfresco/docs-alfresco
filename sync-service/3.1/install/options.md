...
The Sync Service distribution zip file, `AlfrescoSyncServer-3.1.x.zip`, includes all the files required to provide the Sync Service. This file contains the following artifacts:

* `amps-repository` directory containing the Sync Service repository AMP: `alfresco-device-sync-repo-3.1.x.amp`
* `licenses` directory containing the 3rd-party licenses
* `service-sync` directory with:
  * `service-sync-3.1.x.jar` Sync Service JAR
...
4. Use the Module Management Tool (MMT) to install the `alfresco-device-sync-repo-3.1.x.amp` AMP into the repository WAR.

    For more information, see instructions in [Install the AMP file]({% link content-services/6.1/install/zip/amp.md %}).

    For example, to apply the `alfresco-device-sync-repo-3.1.x.amp`, use the following command:

    ```java
    java -jar <alfrescoInstallLocation>\bin\alfresco-mmt.jar install <installLocation>\amps-repository\alfresco-device-sync-repo-3.1.x.amp <installLocation>\tomcat\webapps\alfresco.war
    ```
...
8. Start ActiveMQ.
    ...
    For more information, see [Setting up ActiveMQ]({% link content-services/6.1/config/activemq.md %}).
...
* [Install with Helm charts]({% link content-services/6.1/install/containers/helm.md %})
* [Install with Docker Compose]({% link content-services/6.1/install/containers/docker-compose.md %})
...
2. Uninstall the Sync Service, AMP file in the repository, for example using the Module Management Tool (MMT):
    ...
    ```java
    java -jar bin/alfresco-mmt.jar uninstall alfresco-device-sync-repo-3.1.x.amp tomcat/webapps/alfresco.war
    ```

    [Uninstall an AMP file]({% link content-services/6.1/install/zip/amp.md %}#uninstall-an-amp-file) provides information on how to uninstall the AMP file, and remove the AMP content from the WAR files.
