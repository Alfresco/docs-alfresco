...
The Sync Service distribution zip file, `AlfrescoSyncServer-3.4.x.zip`, includes all the files required to provide the Sync Service. This file contains the following artifacts:

* `amps-repository` directory containing the Sync Service repository AMP: `alfresco-device-sync-repo-3.4.x.amp`
...
> **Note:** Make sure you're running the correct versions of operating system and software before you install the AMP file. See [Prerequisites]({% link sync-service/3.4/install/index.md %}) for more information.

1. Download `AlfrescoSyncServer-3.4.x.zip` from [Hyland Community](https://community.hyland.com/){:target="_blank"}.

2. Extract the `AlfrescoSyncServer-3.4.x.zip` file into a system directory; for example, `<installLocation>/`.
...
4. Use the Module Management Tool (MMT) to install the `alfresco-device-sync-repo-3.4.x.amp` AMP into the repository WAR.

    For more information, see instructions in [Install the AMP file]({% link content-services/latest/install/zip/amp.md %}).

    For example, to apply the `alfresco-device-sync-repo-3.4.x.amp`, use the following command:

    ```java
    java -jar <alfrescoInstallLocation>\bin\alfresco-mmt.jar install <installLocation>\amps-repository\alfresco-device-sync-repo-3.4.x.amp <installLocation>\tomcat\webapps\alfresco.war
    ```
...
6. Configure the Sync Service properties in the `<installLocation>/service-sync/config.yml` file.
    ...
    See [Configure Sync Service]({% link sync-service/3.4/config/index.md %}).
...
7. Start and configure PostgreSQL.
    ...
    For more information, see [Configuring PostgreSQL database for Desktop Sync]({% link sync-service/3.4/install/database.md %}).
...
10. Start the Sync Service.
    ...
    java -Xmx2G -Djava.io.tmpdir=/var/tmp/dsync -classpath <classpath to database.jar file>:service-sync-3.4.x.jar org.alfresco.service.sync.dropwizard.SyncService server config.yml
    ```

    See [Running Sync Service via a script]({% link sync-service/3.4/config/script.md %}).
...
12. Check the repository and Sync Service log file (`<installLocation>/service-sync/logs/sync-service.log` by default) to see if the Sync Service started properly. The location of the log file can be configured using the `logging` properties in the `config.yml` file.
    ...
    To validate that the Sync Service is configured correctly, see [Sync Service health check]({% link sync-service/3.4/admin/monitor/index.md %}#sync-service-health-check).
...
See instructions to [install and configure databases]({% link sync-service/3.4/install/database.md %}).
...
2. Uninstall the Sync Service, AMP file in the repository, for example using the Module Management Tool (MMT):
    ...
    ```java
    java -jar bin/alfresco-mmt.jar uninstall alfresco-device-sync-repo-3.4.x.amp tomcat/webapps/alfresco.war
    ```
