...
Before you start the Sync Service, see the [Prerequisites]({% link sync-service/latest/install/index.md %}) and `Readme.txt` file included in the `AlfrescoSyncServer-3.3.x.zip` for more information.
...
1. Create a user that you'll use to run the process.
    ...
    2. For Windows: You only need a dedicated user if JMX remote authentication is enabled. See `Readme.txt` file in `AlfrescoSyncServer-3.3.x.zip` for details.
